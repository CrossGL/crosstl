...
Supported Languages:
  cuda, metal, directx, opengl, vulkan, rust, mojo, hip, slang, crossgl
    ...
    parser.add_argument('input', nargs='?', 
                       help='Input file path or source code')
    ...
    parser.add_argument('--info', action='store_true',
                       help='Show CrossTL system information')
    ...
    parser.add_argument('--version', action='version', 
                       version=f'CrossTL {crosstl.__version__}')
    parser.add_argument('--check-syntax', action='store_true',
                       help='Check syntax without translation')
...
def validate_arguments(args):
    ...
    if not args.input or not args.target:
        print("Error: Both input and target language are required for translation", file=sys.stderr)
        return False
