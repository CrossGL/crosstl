...
\email{\{nripesh, vaatsalya, bhushan, sushant\}@crossgl.net}}
...
\node[astbox, right=28mm of glslast] (commonast) {Common AST\\(30 base nodes)};
\node[unibox, right=28mm of commonast] (uniast) {Universal AST\\(60+ node types)};
...
The evaluation exposes three categories of limitations that bound the current system's applicability.

\textbf{Uneven backend maturity.} GLSL, Metal, CUDA, HIP, Rust, and Mojo handled all compatible corpus examples, while Slang, HLSL, and SPIR-V exhibit gaps on generic pattern matching, enum lowering, or complex block traversal. This unevenness is inherent to the incremental development model: backends mature at different rates depending on the complexity of their target language semantics.

\textbf{Semantic loss in two-hop translation.} Cross-translation between two non-CrossGL languages routes through CrossGL as a pivot, and this intermediate step may discard target-specific idioms. For instance, HLSL's precise semantics for resource binding slots or Metal's explicit address-space annotations may be approximated rather than preserved exactly when translating from a third language through the IR. However, any improvement to the CrossGL IR benefits all translation paths simultaneously.

\textbf{Structural correctness without runtime equivalence.} The evaluation measures whether generated code is syntactically well-formed and structurally complete, but does not verify pixel-level equivalence of rendered output or bitwise identity of compute results. The SPIR-V backend illustrates this boundary: it emits assembly for six of seven compatible examples, but the generated modules do not yet pass \texttt{spirv-val}, indicating structural rather than semantic correctness at the binary interchange level.

\subsection{Future Work}

Several directions follow from the current results, guided by established principles of structured software design for parallel and heterogeneous systems~\cite{mccool2012structured}.

\textbf{Runtime validation harness.} The highest priority is an automated pipeline that compiles generated GLSL, HLSL, Metal, and SPIR-V through platform toolchains (glslangValidator, fxc/dxc, metal compiler, spirv-val) and compares rendered images or compute buffers within numerical tolerances. This would elevate the evaluation from structural correctness to semantic equivalence.

\textbf{Optimization passes.} The visitor pattern already present in the universal AST facilitates dead code elimination, constant folding, and common subexpression elimination as reusable passes that benefit all backends simultaneously.

\textbf{Extended target coverage.} WGSL support for WebGPU, direct binary SPIR-V and DXIL emission, and extensions for heterogeneous AI accelerators (NPUs, TPUs) would broaden applicability to emerging compute platforms.

\textbf{Developer tooling.} A Language Server Protocol (LSP) implementation for CrossGL would enable syntax highlighting, autocompletion, and error diagnostics in standard editors, lowering the barrier for direct authorship of cross-platform shaders.

\section{Conclusion}

This paper presented CrossGL, a compiler infrastructure that addresses GPU shading language fragmentation through a hub-and-spoke translation architecture centered on a purpose-built intermediate shader language.

The CrossGL language provides a human-readable, semantically rich IR that captures shader concepts across 16 GPU pipeline stages, including modern ray tracing and mesh shader stages, through a type system supporting generics, pattern matching, and platform-agnostic semantic annotations. The hub-and-spoke architecture reduces the number of required translators from $O(N^2)$ to $O(N)$, with each new language requiring only two converters regardless of how many languages are already supported. In the reference implementation, 767 unit tests passed and 56 of 60 compatible example translations succeeded across nine target backends, including a 489-line cross-platform PBR shader.

The results support the practicality of the architecture while also clarifying its current limits: textual backends are substantially more mature than SPIR-V interchange generation, and advanced constructs such as enums and pattern matching require further backend-specific lowering. The extensible plugin architecture nevertheless gives the system a scalable path forward. As heterogeneous computing architectures proliferate---spanning GPUs, NPUs, TPUs, and specialized AI accelerators---the need for universal translation infrastructure will only grow. The hub-and-spoke model provides a principled foundation for bridging these platforms through a single, shared intermediate representation.
