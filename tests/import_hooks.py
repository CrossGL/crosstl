...
class CaseSensitiveBackendFinder(importlib.abc.MetaPathFinder):
    ...
    def find_spec(self, fullname, path=None, target=None):
        ...
        try:
            ...
            if not fullname.startswith('crosstl.backend.') and not fullname.startswith('tests.test_backend.test_'):
                return None
            ...
            if fullname.startswith('crosstl.backend.') and len(parts) >= 3:
                ...
                if backend_name in self.backend_mappings and parts[2] != self.backend_mappings[backend_name]:
