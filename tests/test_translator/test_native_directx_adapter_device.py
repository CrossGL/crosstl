...
def _write_package(tmp_path: Path) -> tuple[Path, Path, str, bytes]:
    ...
    return package_root, build_root, _execution_symbol(execution_header), artifact


def _harness_source(execute_symbol: str) -> str:
...
def test_directx_adapter_device_fixture_matches_generated_contract(tmp_path):
    ...
    package_root, build_root, execute_symbol, artifact = _write_package(tmp_path)
    descriptor = _descriptor(artifact)
    ...
    assert execute_symbol in _harness_source(execute_symbol)
...
def test_generated_directx_adapter_executes_specialized_hlsl_on_device(tmp_path):
    ...
    package_root, build_root, execute_symbol, _artifact = _write_package(tmp_path)
    include_root = _prepare_dxc_runtime(build_root)
    source_path = build_root / "directx-adapter-device.cpp"
    source_path.write_text(_harness_source(execute_symbol), encoding="utf-8")
