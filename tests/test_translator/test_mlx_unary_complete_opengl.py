...
def test_current_mlx_unary_opengl_contract_is_complete_and_classified() -> None:
    ...
    assert Counter(entry["shape"] for entry in entries) == contract["classifications"][
        "shapes"
    ]
    assert Counter(entry["templateName"] for entry in entries) == contract[
        "classifications"
    ]["templates"]
    assert Counter(entry["operator"] for entry in entries) == contract[
        "classifications"
    ]["operators"]
    assert Counter(
        f'{entry["inputType"]}->{entry["outputType"]}' for entry in entries
    ) == contract["classifications"]["typePairs"]
    assert Counter(entry["family"] for entry in entries) == contract[
        "classifications"
    ]["families"]
...
def test_current_mlx_unary_opengl_ci_shards_are_complete_and_disjoint() -> None:
    ...
    for shard_index, shard in enumerate(shards):
        assert shard == UNARY_OPENGL_WORKLOADS[
            shard_index::UNARY_OPENGL_CI_SHARD_COUNT
        ]
