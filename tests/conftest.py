    ...
    if platform.system() == "Linux":  # Only check on case-sensitive filesystems
        ...
        for dir_path in backend_dirs:
            ...
            if not os.path.isdir(full_path):
                print(f"Warning: {dir_path} directory does not exist or has incorrect case")
                ...
                if os.path.isdir(parent_dir):
                    ...
                    for item in os.listdir(parent_dir):
                        if item.lower() == expected_name.lower() and item != expected_name:
                            print(f"  Found directory with different case: {os.path.join(parent_dir, item)}")
                            print(f"  Please rename to match expected case: {expected_name}")
