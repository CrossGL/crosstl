...
class TestCodeFormatter:
    ...
    def test_format_code_language_detection(self):
        ...
        assert formatter.format_code("code", file_path="shader.hlsl") == "clang_formatted"
        assert formatter.format_code("code", file_path="shader.glsl") == "clang_formatted"
        assert formatter.format_code("code", file_path="shader.metal") == "clang_formatted"
        assert formatter.format_code("code", file_path="shader.spirv") == "spirv_formatted"
        
        # Test with explicit language
        assert formatter.format_code("code", language=ShaderLanguage.HLSL) == "clang_formatted"
        assert formatter.format_code("code", language=ShaderLanguage.SPIRV) == "spirv_formatted"
    ...
    def test_format_with_clang(self):
        ...
        with mock.patch('tempfile.NamedTemporaryFile') as mock_tempfile, \
             ...
             mock.patch('builtins.open', mock.mock_open(read_data="formatted_code")), \
    ...
    def test_validate_spirv(self):
        ...
        with mock.patch('tempfile.NamedTemporaryFile') as mock_tempfile, \
            ...
            mock_run.side_effect = [
                ...
                mock.MagicMock(returncode=1, stderr="Validation error")  # spirv-val fails
            ]
...
def test_format_file():
    # Test successful formatting
    with mock.patch('builtins.open', mock.mock_open(read_data="code")), \
         mock.patch('crosstl.formatter.CodeFormatter.format_code', return_value="formatted_code"):
...
def test_format_shader_code():
    ...
    with mock.patch('crosstl.formatter.CodeFormatter') as MockFormatter:
        ...
        mock_instance.format_code.assert_called_with("code", ShaderLanguage.METAL, "output.metal")
        
        format_shader_code("code", "directx", "output.hlsl")
        mock_instance.format_code.assert_called_with("code", ShaderLanguage.HLSL, "output.hlsl")
        
        format_shader_code("code", "opengl", "output.glsl")
        mock_instance.format_code.assert_called_with("code", ShaderLanguage.GLSL, "output.glsl")
        
        format_shader_code("code", "vulkan", "output.spirv")
        mock_instance.format_code.assert_called_with("code", ShaderLanguage.SPIRV, "output.spirv") 