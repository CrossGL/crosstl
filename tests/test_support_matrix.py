...
def test_project_source_provenance_documents_source_map_mapping_checks():
    ...
    for backend_support in feature["support"].values():
        ...
        assert "source-relative target/variant layout" in backend_support["notes"]
        assert (
            "artifact generated byte-size records that are missing or malformed, "
            "generated-size validation records that no longer match the current "
            "generated artifact"
        ) in backend_support["notes"]
        assert (
            "records per-artifact source hash, generated hash, generated-size, "
            "source-map, and source-remap statuses"
        ) in backend_support["notes"]
        assert (
            "records aggregate validation status counts including generated-size "
            "counts"
        ) in backend_support["notes"]
...
def test_project_validation_hooks_document_migration_contract_checks():
    ...
    for backend_support in feature["support"].values():
        ...
        assert (
            "required summarized validation artifact hash, generated-size, "
            "source-map, and source-remap status fields"
        ) in backend_support["notes"]
        assert "generated-size status counts" in backend_support["notes"]
        assert "generated-size status, and validation status rollups" in (
            backend_support["notes"]
        )
