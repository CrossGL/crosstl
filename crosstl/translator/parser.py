...
class Parser:
        ...
        while self.current_token[0] != "EOF":
            try:
                if self.current_token[0] == "SHADER":
                    ...
                    self.report_error(f"Skipping unexpected token {self.current_token[0]}")
                    self.eat(self.current_token[0])  # Skip unknown tokens
            except Exception as e:
    ...
    def parse_global_variable(self):
        ...
        if self.current_token[0] == "EQUALS":
            ...
            var_node = VariableNode(var_type, var_name) if not is_dynamic_array and array_size is None else ArrayNode(var_type, var_name, array_size)
            self.eat("SEMICOLON")
    ...
    def is_function(self):
        ...
        if current_pos + 1 < len(self.tokens) and self.tokens[current_pos + 1][0] == "EQUALS":
    ...
    def parse_main_function(self):
        ...
        while depth > 0 and self.current_token[0] != "EOF":
            ...
            if ((self.current_token[0] in ["VOID", "FLOAT", "VECTOR", "DOUBLE", "UINT", "INT", "MATRIX", "IDENTIFIER", 
                "STRUCT", "SAMPLER2D", "SAMPLERCUBE", "SAMPLER", "BOOL"]) and 
                (self.pos + 1 < len(self.tokens) and self.tokens[self.pos + 1][0] == "IDENTIFIER")):
                
                # Check if this is the 'main' function or a helper function
                function_name = self.tokens[self.pos + 1][1]
                ...
                if name == "main":
                    ...
                    main_function = FunctionNode(return_type, name, params, body, qualifier=qualifier, semantic=semantic)
                else:
    ...
    def parse_statement(self):
        ...
        try:
            if self.current_token[0] == "IF":
                ...
                if self.current_token[0] == "SEMICOLON":
                    self.eat("SEMICOLON")
                elif self.current_token[0] != "RBRACE" and self.current_token[0] != "EOF":
                    self.report_error(f"Expected ';' after expression, found: {self.current_token[0]}")
                    
                    # Skip to next statement
                    while self.current_token[0] not in ["SEMICOLON", "RBRACE", "RETURN", "IF", "FOR", "WHILE", "EOF"]:
                        self.next_token()
                ...
                self.report_error(f"Unexpected token {self.current_token[0]} in statement, skipping")
    ...
    def parse_if_statement(self):
        ...
        while self.current_token[0] == "ELSE" and self.pos + 1 < len(self.tokens) and self.tokens[self.pos + 1][0] == "IF":
            self.eat("ELSE")
            self.eat("IF")
            self.eat("LPAREN")
        ...
        return IfNode(if_condition, if_body, else_if_conditions, else_if_bodies, else_body)
    ...
    def parse_return_statement(self):
        ...
        if self.current_token[0] == "SEMICOLON":
            ...
            self.report_error(f"Expected ';' after return statement, found: {self.current_token[0]}")
    ...
    def parse_function_call(self, func_name):
        ...
        is_vector_constructor = isinstance(func_name, str) and func_name.startswith('vec')
    ...
    def parse_variable_declaration(self, type_name, update_condition=False):
        ...
        if self.current_token[0] == "IDENTIFIER":
            ...
            if self.current_token[0] == "EQUALS":
                ...
                if self.current_token[0] == "SEMICOLON":
                    ...
                    if not update_condition and self.current_token[0] not in ["RBRACE", "EOF"]:
                        self.report_error(f"Expected ';' after variable assignment, found: {self.current_token[0]}")
                        
                        # Try to move to next statement
                        if self.current_token[0] not in ["RETURN", "IF", "FOR", "WHILE", "SEMICOLON", "RBRACE", "EOF"]:
                ...
                if self.current_token[0] == "SEMICOLON":
                    ...
                    if self.current_token[0] != "RBRACE":
                        self.report_error(f"Expected ';' after variable declaration, found: {self.current_token[0]}")
        ...
        # Handle initialization with equals
            ...
            if self.current_token[0] == "SEMICOLON":
                ...
                if update_condition:
                    ...
                    if self.current_token[0] not in ["SEMICOLON", "COMMA", "RPAREN", "RBRACE", "EOF"]:
                        while self.current_token[0] not in ["SEMICOLON", "RBRACE", "RETURN", "IF", "FOR", "WHILE", "EOF"]:
                            self.next_token()
        ...
        # Handle LPAREN case (function calls as declaration)
            ...
            if self.current_token[0] == "SEMICOLON":
                ...
                if self.current_token[0] == "SEMICOLON":
                    ...
                    self.report_error(f"Expected ';' after assignment, found: {self.current_token[0]}")
                
                return BinaryOpNode(func_call, op, value)
            else:
                # Allow more relaxed syntax for function calls
                if self.current_token[0] != "RBRACE":
                    self.report_error(f"Expected ';' after function call, found: {self.current_token[0]}")
                return func_call
        ...
        # Handle other operators
            ...
            self.report_error(f"Unexpected token in variable declaration: {self.current_token[0]}")
    ...
    def parse_assignment(self, name):
        ...
        if self.current_token[0] in [
            ...
            self.report_error(f"Expected assignment operator, found: {self.current_token[0]}")
        ...
        if self.current_token[0] == "IDENTIFIER":
            ...
            try:
                ...
                if self.current_token[0] == "RPAREN":
                    ...
                    self.report_error(f"Expected closing parenthesis, found: {self.current_token[0]}")
                    # Look for common delimiters
                    while (self.current_token[0] not in 
                          ["RPAREN", "SEMICOLON", "COMMA", "RBRACE", "EOF"]):
                        self.next_token()
            ...
            self.report_error(f"Unexpected token in expression: {self.current_token[0]}")
    ...
    def parse_expression(self):
        ...
        try:
            ...
            recovery_tokens = ["SEMICOLON", "COMMA", "RPAREN", "RBRACE", "RBRACKET", "EOF"]
    ...
    def parse_function_call_or_identifier(self):
        ...
        if self.current_token[0] == "LPAREN":
            ...
            if self.current_token[0] == "RBRACKET":
                ...
                logging.warning(f"Expected ']' after array index, found: {self.current_token[0]}")
        ...
        while self.current_token[0] in ["DOT", "LBRACKET"]:
            if self.current_token[0] == "DOT":
                try:
                    self.eat("DOT")
                    if self.current_token[0] != "IDENTIFIER":
                        self.report_error(f"Expected identifier after '.', found: {self.current_token[0]}")
                        break
                        ...
                        if self.current_token[0] != "RPAREN":
                            while True:
                                ...
                                if self.current_token[0] == "COMMA":
                                    ...
                                    self.report_error(f"Expected ',' or ')' in method call, found: {self.current_token[0]}")
                                    # Skip to the next delimiter
                                    while self.current_token[0] not in ["COMMA", "RPAREN", "SEMICOLON", "EOF"]:
                                        self.next_token()
                ...
                try:
                    ...
                    if self.current_token[0] == "RBRACKET":
                        ...
                        self.report_error(f"Expected ']' after array index, found: {self.current_token[0]}")
                        # Try to recover
                        while self.current_token[0] not in ["RBRACKET", "SEMICOLON", "COMMA", "RPAREN", "EOF"]:
                            self.next_token()
    ...
    def parse_binary_expression(self, left, min_precedence=0):
        ...
        precedence = {
            ...
            "MULTIPLY": 13, "DIVIDE": 13, "MODULO": 13,
            # Additive operators
            "PLUS": 12, "MINUS": 12,
            # Shift operators
            "LEFT_SHIFT": 11, "RIGHT_SHIFT": 11,
            # Relational operators
            "LESS_THAN": 10, "LESS_THAN_EQUAL": 10, "GREATER_THAN": 10, "GREATER_THAN_EQUAL": 10,
            # Equality operators
            "EQUAL": 9, "NOT_EQUAL": 9,
            # Bitwise operators
            "BITWISE_AND": 8, "BITWISE_XOR": 7, "BITWISE_OR": 6,
            # Logical operators
            "LOGICAL_AND": 5, "LOGICAL_OR": 4,
            # Assignment operators (lowest precedence)
            "ASSIGN": 1, "PLUS_ASSIGN": 1, "MINUS_ASSIGN": 1, "MULTIPLY_ASSIGN": 1,
            "DIVIDE_ASSIGN": 1, "MODULO_ASSIGN": 1, "LEFT_SHIFT_ASSIGN": 1, "RIGHT_SHIFT_ASSIGN": 1,
            "BITWISE_AND_ASSIGN": 1, "BITWISE_XOR_ASSIGN": 1, "BITWISE_OR_ASSIGN": 1
        }
        ...
        operator_map = {
            ...
            "DIVIDE_ASSIGN": "/=", "MODULO_ASSIGN": "%=", "LEFT_SHIFT_ASSIGN": "<<=", "RIGHT_SHIFT_ASSIGN": ">>=",
            "BITWISE_AND_ASSIGN": "&=", "BITWISE_XOR_ASSIGN": "^=", "BITWISE_OR_ASSIGN": "|="
        }

        while (self.current_token[0] in precedence and
               precedence[self.current_token[0]] >= min_precedence):
            ...
            while (self.current_token[0] in precedence and
                   (precedence[self.current_token[0]] > op_precedence or
                    (precedence[self.current_token[0]] == op_precedence and 
                     self.current_token[0] not in ["ASSIGN", "PLUS_ASSIGN", "MINUS_ASSIGN", 
                                                  "MULTIPLY_ASSIGN", "DIVIDE_ASSIGN", "MODULO_ASSIGN",
                                                  "LEFT_SHIFT_ASSIGN", "RIGHT_SHIFT_ASSIGN",
                                                  "BITWISE_AND_ASSIGN", "BITWISE_XOR_ASSIGN", "BITWISE_OR_ASSIGN"]))):
                right = self.parse_binary_expression(right, precedence[self.current_token[0]])
