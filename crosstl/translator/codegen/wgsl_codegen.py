...
class WGSLCodeGen:
    ...
    STORAGE_TEXTURE_TYPE_MAP = {
        ...
        "image2d": "texture_storage_2d",
        "image2darray": "texture_storage_2d_array",
        "image3d": "texture_storage_3d",
        "imagecube": "texture_storage_2d_array",
        "imagecubearray": "texture_storage_2d_array",
        "iimage1d": "texture_storage_1d",
        "iimage2d": "texture_storage_2d",
        "iimage2darray": "texture_storage_2d_array",
        "iimage3d": "texture_storage_3d",
        "iimagecube": "texture_storage_2d_array",
        "iimagecubearray": "texture_storage_2d_array",
        "uimage1d": "texture_storage_1d",
        "uimage2d": "texture_storage_2d",
        "uimage2darray": "texture_storage_2d_array",
        "uimage3d": "texture_storage_3d",
        "uimagecube": "texture_storage_2d_array",
        "uimagecubearray": "texture_storage_2d_array",
    }
    ...
    STORAGE_TEXTURE_DEFAULT_FORMATS = {
        "image1d": "rgba32float",
        "image2d": "rgba32float",
        "image2darray": "rgba32float",
        "image3d": "rgba32float",
        "imagecube": "rgba32float",
        "imagecubearray": "rgba32float",
        "iimage1d": "rgba32sint",
        "iimage2d": "rgba32sint",
        "iimage2darray": "rgba32sint",
        "iimage3d": "rgba32sint",
        "iimagecube": "rgba32sint",
        "iimagecubearray": "rgba32sint",
        "uimage1d": "rgba32uint",
        "uimage2d": "rgba32uint",
        "uimage2darray": "rgba32uint",
        "uimage3d": "rgba32uint",
        "uimagecube": "rgba32uint",
        "uimagecubearray": "rgba32uint",
    }
    ...
    SAMPLER_TYPE_NAMES = REGULAR_SAMPLER_TYPE_NAMES | COMPARISON_SAMPLER_TYPE_NAMES
    ...
    def __init__(self):
        ...
        self._generic_struct_definitions = {}
        self._generic_struct_specializations = {}
        ...
        self._stage_interface_struct_names = set()
        self._stage_interface_builtin_members = {}
    ...
    def generate_program(self, ast, target_stage=None):
        ...
        source_structs = self._collect_structs(ast, target_stage)
        self._generic_struct_definitions = self.collect_generic_struct_definitions(
            source_structs
        )
        structs = self.monomorphized_structs(ast, target_stage, source_structs)
    ...
    def stage_interface_member_locations(self, ast, target_stage, structs):
        ...
        self._stage_interface_struct_names = set(interface_struct_names)
        locations = {}
        for struct in structs:
            ...
            for member in getattr(struct, "members", []) or []:
                if (struct_name, member.name) in self._stage_interface_builtin_members:
                    continue
                binding_kind, location = self.member_stage_binding(member)
                slot_count = self.stage_interface_location_slot_count(
                    getattr(member, "member_type", None)
                )
                if binding_kind == "location":
                    if location is not None:
                        used_locations.update(range(location, location + slot_count))
                    continue
            ...
            for member in unbound_members:
                ...
                while any(
                    location in used_locations
                    for location in range(next_location, next_location + slot_count)
                ):
                    next_location += 1
                locations[(struct_name, member.name)] = next_location
                used_locations.update(range(next_location, next_location + slot_count))
                next_location += slot_count
        return locations

    def stage_interface_location_slot_count(self, member_type):
        info = self.stage_interface_array_member_info_for_type(member_type)
        if info is not None:
            return info["size"]
        info = self.stage_interface_matrix_member_info_for_type(member_type)
        if info is not None:
            return info["columns"]
        return 1

    def stage_interface_builtin_members(self, ast, target_stage, structs_by_name):
        builtin_members = {}
        for stage_node in self._stage_nodes(ast, target_stage):
            stage_name = normalize_stage_name(getattr(stage_node, "stage", None))
            ...
            if not struct_name:
                continue
            struct = structs_by_name.get(struct_name)
            if struct is None:
                continue
            for member in getattr(struct, "members", []) or []:
                binding_kind, _location = self.member_stage_binding(member)
                if binding_kind is not None:
                    continue
                if self.is_vertex_position_member_name(getattr(member, "name", "")):
                    builtin_members[(struct_name, member.name)] = "position"
        return builtin_members

    def is_vertex_position_member_name(self, name):
        return self.semantic_key(name) in {"position", "clip_position", "clipposition"}
    ...
    def generate_struct(self, node):
        if getattr(node, "generic_params", None):
            raise ValueError(
                "WGSL target cannot emit unspecialized generic struct "
                f"{node.name}; use concrete type arguments before WGSL generation"
            )
        ...
        for member in node.members:
            ...
            stage_array = self.stage_interface_array_member_info_for_type(member_type)
            base_location = (
                implicit_location
                if implicit_location is not None
            ...
            if stage_array is not None and base_location is not None:
                for index in range(stage_array["size"]):
                    field_name = self.stage_interface_array_field_name(
                        node.name, member.name, index
                    )
                    if "@location(" in attributes:
                        field_attributes = re.sub(
                            r"@location\(\d+\)",
                            f"@location({base_location + index})",
                            attributes,
                            count=1,
                        )
                    else:
                    ...
                    prefix = f"{field_attributes} " if field_attributes else ""
                    lines.append(
                        f"    {prefix}{field_name}: {stage_array['element_type_name']},"
                    )
                continue
            stage_matrix = self.stage_interface_matrix_member_info_for_type(member_type)
            if stage_matrix is not None and base_location is not None:
                for index in range(stage_matrix["columns"]):
                    field_name = self.stage_interface_array_field_name(
                        node.name, member.name, index
                    )
                    if "@location(" in attributes:
                    ...
                    prefix = f"{field_attributes} " if field_attributes else ""
                    lines.append(
                        f"    {prefix}{field_name}: {stage_matrix['column_type_name']},"
                    )
                continue
    ...
    def collect_generic_struct_definitions(self, structs):
        return {
            struct.name: struct
            for struct in structs
    ...
    def monomorphized_structs(self, ast, target_stage, source_structs):
        concrete_structs = [
            struct
            for struct in source_structs
            if not getattr(struct, "generic_params", None)
        ]
        self._generic_struct_specializations = {}
        self._generic_struct_specialized_names = {}

        if not self._generic_struct_definitions:
            return concrete_structs

        generic_parameter_names = self.generic_parameter_names(ast)
        pending = []

        def enqueue_specialization(base_name, generic_args):
            if base_name not in self._generic_struct_definitions:
                return
            if not generic_args:
                return
            vtype = NamedType(base_name, list(generic_args))
            if self.type_node_contains_generic_parameter(
                vtype, generic_parameter_names
            ):
                return
            key = self.generic_struct_type_key(vtype.name, vtype.generic_args)
            if (
                key in self._generic_struct_specializations
        ...
        def enqueue_from_ast_value(value):
            if isinstance(value, NamedType):
                enqueue_specialization(value.name, value.generic_args)
                return
            if not isinstance(value, FunctionCallNode):
                return
            function = getattr(value, "function", None)
            if not isinstance(function, IdentifierNode):
                return
            parsed = self.generic_constructor_parts(function.name)
            if parsed is None:
                return
            enqueue_specialization(*parsed)

        self.visit_ast_values(ast, enqueue_from_ast_value)
        index = 0
        while index < len(pending):
            vtype = pending[index]
            index += 1
            key = self.generic_struct_type_key(vtype.name, vtype.generic_args)
            if key in self._generic_struct_specializations:
                continue
            specialization = self.build_generic_struct_specialization(vtype, key)
            self._generic_struct_specializations[key] = specialization
            self.visit_ast_values(specialization, enqueue_from_ast_value)

        return concrete_structs + list(self._generic_struct_specializations.values())

    def build_generic_struct_specialization(self, vtype, key):
        definition = self._generic_struct_definitions[vtype.name]
        generic_params = [
            param.name
            for param in getattr(definition, "generic_params", []) or []
            if getattr(param, "name", None)
        ]
        if len(vtype.generic_args) != len(generic_params):
            expected = len(generic_params)
            actual = len(vtype.generic_args)
            raise ValueError(
                "WGSL target cannot specialize generic struct "
                f"{vtype.name}; expected {expected} type argument"
                f"{'s' if expected != 1 else ''}, got {actual}"
            )

        substitutions = {
            param: copy.deepcopy(arg)
            for param, arg in zip(generic_params, vtype.generic_args)
        }
        members = []
        for member in getattr(definition, "members", []) or []:
            if not isinstance(member, StructMemberNode):
                raise ValueError(
                    "WGSL target cannot specialize generic struct "
                    f"{definition.name}; member {getattr(member, 'name', '<unnamed>')} "
                    f"uses unsupported {type(member).__name__}"
                )
            specialized = copy.copy(member)
            specialized.member_type = self.substitute_generic_struct_type(
                member.member_type, substitutions
            )
            members.append(specialized)

        return StructNode(
            self._generic_struct_specialized_names[key],
            members,
            attributes=list(getattr(definition, "attributes", []) or []),
            inheritance=list(getattr(definition, "inheritance", []) or []),
            visibility=getattr(definition, "visibility", "public"),
        )

    def substitute_generic_struct_type(self, vtype, substitutions):
        if isinstance(vtype, NamedType):
            if not vtype.generic_args and vtype.name in substitutions:
                return copy.deepcopy(substitutions[vtype.name])
            return NamedType(
                vtype.name,
                [
                    self.substitute_generic_struct_type(arg, substitutions)
                    for arg in vtype.generic_args
                ],
            )
        if isinstance(vtype, VectorType):
            return VectorType(
                self.substitute_generic_struct_type(vtype.element_type, substitutions),
                vtype.size,
            )
        ...
        if isinstance(vtype, ArrayType):
            return ArrayType(
                self.substitute_generic_struct_type(vtype.element_type, substitutions),
                copy.deepcopy(vtype.size),
            )
        if isinstance(vtype, PointerType):
            return PointerType(
                self.substitute_generic_struct_type(vtype.pointee_type, substitutions),
                is_mutable=vtype.is_mutable,
            )
        if isinstance(vtype, ReferenceType):
            return ReferenceType(
                self.substitute_generic_struct_type(
                    vtype.referenced_type, substitutions
                ),
                is_mutable=vtype.is_mutable,
            )
        if isinstance(vtype, GenericType) and vtype.name in substitutions:
            return copy.deepcopy(substitutions[vtype.name])
        return copy.deepcopy(vtype)

    def visit_ast_values(self, value, callback):
        visited = set()
    ...
    def generic_parameter_names(self, value):
        names = set()
        ...
        self.visit_ast_values(value, collect)
        return names

    def type_node_contains_generic_parameter(self, vtype, generic_parameter_names):
        if isinstance(vtype, str):
            key = self.generic_type_key(vtype)
            if key in generic_parameter_names:
                return True
            parsed = self.generic_constructor_parts(vtype)
            if parsed is None:
                return False
        ...
        if isinstance(vtype, NamedType):
            if not vtype.generic_args and vtype.name in generic_parameter_names:
                return True
            return any(
                self.type_node_contains_generic_parameter(arg, generic_parameter_names)
                for arg in vtype.generic_args
        ...
        if isinstance(vtype, GenericType):
            return vtype.name in generic_parameter_names
        if isinstance(vtype, VectorType):
            return self.type_node_contains_generic_parameter(
                vtype.element_type, generic_parameter_names
            )
        if isinstance(vtype, MatrixType):
            return self.type_node_contains_generic_parameter(
                vtype.element_type, generic_parameter_names
        ...
        if isinstance(vtype, PointerType):
            return self.type_node_contains_generic_parameter(
                vtype.pointee_type, generic_parameter_names
            )
        if isinstance(vtype, ReferenceType):
            return self.type_node_contains_generic_parameter(
                vtype.referenced_type, generic_parameter_names
            )
        return False

    def generic_struct_type_key(self, base_name, generic_args):
        return (
            str(base_name),
            tuple(self.generic_type_key(arg) for arg in generic_args),
        )

    def generic_type_key(self, vtype):
        if isinstance(vtype, PrimitiveType):
            return self.PRIMITIVE_TYPE_MAP.get(vtype.name.lower(), vtype.name)
        if isinstance(vtype, VectorType):
            return f"vec{vtype.size}<{self.generic_type_key(vtype.element_type)}>"
        if isinstance(vtype, MatrixType):
            return (
                f"mat{vtype.cols}x{vtype.rows}<"
                f"{self.generic_type_key(vtype.element_type)}>"
            )
        if isinstance(vtype, ArrayType):
            element = self.generic_type_key(vtype.element_type)
            if vtype.size is None:
                return f"array<{element}>"
            return f"array<{element}, {vtype.size}>"
        if isinstance(vtype, PointerType):
            return f"ptr<{self.generic_type_key(vtype.pointee_type)}>"
        if isinstance(vtype, ReferenceType):
            return self.generic_type_key(vtype.referenced_type)
        if isinstance(vtype, NamedType):
            if not vtype.generic_args:
                return self.map_type_name(vtype.name)
            args = ", ".join(self.generic_type_key(arg) for arg in vtype.generic_args)
            return f"{vtype.name}<{args}>"
        if isinstance(vtype, GenericType):
            return vtype.name
        if isinstance(vtype, str):
            ...
            if match is not None:
                ...
                if args is not None:
                    rendered_args = ", ".join(
                        self.generic_type_key(arg) for arg in args
                    )
                    return f"{match.group(1)}<{rendered_args}>"
        return self.map_type_name(str(vtype))

    def generic_struct_specialized_name_from_key(self, key):
        base_name, generic_args = key
    ...
    def generic_struct_specialized_type_name(self, vtype):
        generic_parameter_names = self.generic_parameter_names(
            self._generic_struct_definitions.get(vtype.name)
        )
        if self.type_node_contains_generic_parameter(vtype, generic_parameter_names):
            args = ", ".join(self.generic_type_key(arg) for arg in vtype.generic_args)
            raise ValueError(
                "WGSL target cannot specialize generic struct "
                f"{vtype.name}<{args}> because at least one type argument is "
                "still generic"
            )
        key = self.generic_struct_type_key(vtype.name, vtype.generic_args)
        if key not in self._generic_struct_specialized_names:
            raise ValueError(
                "WGSL target cannot emit generic struct "
                f"{self.generic_type_key(vtype)}; no concrete specialization was "
                "collected before WGSL generation"
            )
        return self.type_identifier_name(self._generic_struct_specialized_names[key])
    ...
    def generate_global_variable(self, node):
        ...
        storage_texture_type = self.storage_texture_type(node.var_type, node=node)
    ...
    def generate_storage_texture_global_variable(self, node, texture_type):
        ...
        self._module_storage_access_modes[node.name] = self.storage_texture_access(node)
        return (
            f"{attributes}\nvar {self.module_identifier_name(node.name)}: "
            f"{texture_type};"
        )
    ...
    def generate_assignment_statement(self, node, indent):
        ...
        pad = "    " * indent
        return f"{pad}{self.generate_assignment(node)};"

    def generate_stage_interface_array_assignment_statement(self, node, indent):
        target = getattr(node, "target", None)
        access = self.stage_interface_array_access(target)
        if access is None:
            return None

        target_type = self.expression_type(target)
        value = self.generate_expression_for_target(node.value, target_type)
        index = self.resolve_integer_expression(target.index_expr)
        pad = "    " * indent
        if index is not None:
            if index < 0 or index >= access["size"]:
                raise ValueError(
                    "WGSL target stage interface array "
                    f"{access['member_name']} index {index} is outside the lowered "
                    f"field range 0..{access['size'] - 1}"
                )
            field = self.stage_interface_array_access_field(access, index)
            return f"{pad}{field} {node.operator} {value};"

        index_expr = self.generate_expression(target.index_expr)
        lines = [f"{pad}switch ({index_expr}) {{"]
        for field_index in range(access["size"]):
            field = self.stage_interface_array_access_field(access, field_index)
            lines.append(f"{pad}    case {field_index}: {{")
            lines.append(f"{pad}        {field} {node.operator} {value};")
            lines.append(f"{pad}        break;")
            lines.append(f"{pad}    }}")
        lines.append(f"{pad}    default: {{")
        lines.append(f"{pad}        break;")
    ...
    def generate_stage_interface_matrix_assignment_statement(self, node, indent):
        target = getattr(node, "target", None)
        access = self.stage_interface_matrix_access(target)
        if access is None:
            return None
        ...
        value = self.generate_expression_for_target(node.value, target_type)
        pad = "    " * indent
        temp_name = self.declare_synthetic_local_identifier(
            self.swizzle_base_name(access["member_name"])
        )
        lines = [
            f"{pad}let {temp_name}: {self.type_name_string(target_type)} = {value};"
        ]
        for index in range(access["columns"]):
            field = self.stage_interface_matrix_access_field(access, index)
            lines.append(f"{pad}{field} {node.operator} {temp_name}[{index}];")
        return "\n".join(lines)

    def generate_swizzle_assignment_statement(self, node, indent):
        target = getattr(node, "target", None)
        target_info = self.swizzle_assignment_target_info(target)
        if target_info is None:
            return None
        base, components = target_info
        if not self.assignable_swizzle_components(components):
            return None

        pad = "    " * indent
        target_type = self.expression_type(target)
        value = self.generate_expression_for_target(node.value, target_type)
        value_type = self.expression_type(node.value)

        if node.operator != "=":
            lines = []
            for index, component in enumerate(components):
                rhs = self.swizzle_assignment_value_component(
                    value, value_type, index, len(components)
                )
                lines.append(f"{pad}{base}.{component} {node.operator} {rhs};")
            return "\n".join(lines)

        temp_name = self.declare_synthetic_local_identifier(
            f"{self.swizzle_base_name(base)}_{''.join(components)}"
        )
        lines = [
            f"{pad}let {temp_name}: {self.type_name_string(target_type)} = {value};"
        ]
        for index, component in enumerate(components):
            rhs = self.swizzle_assignment_value_component(
                temp_name, target_type, index, len(components)
            )
            lines.append(f"{pad}{base}.{component} = {rhs};")
        return "\n".join(lines)

    def swizzle_assignment_target_info(self, target):
        if isinstance(target, SwizzleNode) and len(target.components) > 1:
            return self.generate_expression(target.vector_expr), list(target.components)
        if isinstance(target, MemberAccessNode) and len(target.member) > 1:
            object_type = self.expression_type(target.object_expr)
            if self.vector_component_type(object_type) is None:
                return None
            if not self.is_vector_member(target.member):
                return None
            return self.generate_expression(target.object_expr), list(target.member)
        return None

    def assignable_swizzle_components(self, components):
        if len(set(components)) != len(components):
            raise ValueError(
                "WGSL target cannot lower assignment to swizzle with duplicate "
                f"components {components}"
            )
        return all(component in "xyzw" for component in components)

    def swizzle_assignment_value_component(
    ...
    def swizzle_base_name(self, base):
        return re.sub(r"[^0-9A-Za-z_]+", "_", base).strip("_") or "swizzle"

    def declare_synthetic_local_identifier(self, base):
        used_names = set()
        for scope in self._identifier_scopes:
            used_names.update(scope)
        for aliases in self._identifier_alias_scopes:
            used_names.update(aliases.values())
        emitted_name = self.unique_wgsl_identifier(
            self.safe_wgsl_identifier(base), used_names
        )
        if self._identifier_scopes:
            self._identifier_scopes[-1].add(emitted_name)
        return emitted_name
    ...
    def generate_expression_for_target(self, expr, target_type):
        ...
        if narrowed is not None:
            return narrowed
        target_scalar = self.scalar_type_name(target_type)
        source_scalar = self.scalar_type_name(source_type)
        ...
        if target_scalar == "f32" and source_scalar in {"i32", "u32"}:
            return f"f32({rendered})"
        return rendered
    ...
    def generate_expression(self, expr):
        ...
        if isinstance(expr, IdentifierNode):
            ...
            if not self.is_local_identifier(expr.name):
                ...
                if cbuffer_access:
                    if self.is_bool_type(self._cbuffer_member_types.get(expr.name)):
                        return f"({cbuffer_access} != 0u)"
        ...
        if isinstance(expr, MemberAccessNode):
            ...
            stage_matrix = self.generate_stage_interface_matrix_access(expr)
        ...
        if isinstance(expr, ArrayAccessNode):
            ...
            stage_array = self.generate_stage_interface_array_access(expr)
            if stage_array is not None:
                return stage_array
            if isinstance(
    ...
    def generate_function_call(self, node, expected_type=None):
        ...
        if normalized_name == "clamp":
            return self.generate_clamp_call(node)
    ...
    def generate_clamp_call(self, node):
        if len(node.arguments) != 3:
            raise ValueError(
                "WGSL target supports clamp() calls with exactly 3 arguments; got "
                f"{len(node.arguments)}"
            )
        value_expr = self.generate_expression(node.arguments[0])
        value_type = self.expression_type(node.arguments[0])
        vector_type_name = self.vector_type_name(value_type)
        if vector_type_name is None:
            return (
                "clamp("
                + ", ".join(self.generate_expression(arg) for arg in node.arguments)
                + ")"
            )
        lower = self.generate_clamp_bound(node.arguments[1], vector_type_name)
        upper = self.generate_clamp_bound(node.arguments[2], vector_type_name)
        return f"clamp({value_expr}, {lower}, {upper})"

    def generate_clamp_bound(self, bound, vector_type_name):
        rendered = self.generate_expression(bound)
        if self.vector_component_count(self.expression_type(bound)) is not None:
            return rendered
        return f"{vector_type_name}({rendered})"
    ...
    def generate_texture_function_call(self, node, function_name):
        ...
        if normalized_name == "texturecompare":
            return self.generate_texture_compare_call(function_name, args)
    ...
    def generate_texel_fetch_call(self, function_name, args):
        if len(args) != 3:
            raise ValueError(
                "WGSL target supports texelFetch() calls with texture, coordinate, "
                f"and mip/sample operands; got {len(args)} argument(s) for "
                f"{function_name}"
            )
    ...
    def generate_image_store_call(self, function_name, args):
        if len(args) != 3:
            raise ValueError(
                "WGSL target supports imageStore() calls with image, coordinate, "
                f"and value operands; got {len(args)} argument(s) for "
                f"{function_name}"
            )
        image, coords, value = args
        return (
    ...
    def generate_texture_compare_call(self, function_name, args):
        if len(args) == 3:
            ...
            raise ValueError(
                ...
                "texture/coords/depth or texture/sampler/coords/depth arguments; "
                f"got {len(args)} argument(s) for {function_name}"
            )

        if not self.is_comparison_texture_expression(texture):
            raise ValueError(
                "WGSL target supports textureCompare() only for shadow/depth "
    ...
    def generate_texture_gather_call(self, function_name, args):
        ...
        if len(args) == 2:
            ...
            if self.is_sampler_expression(args[1]):
                sampler, coords = args[1:]
            else:
                coords, component = args[1:]
                sampler = None
        elif len(args) == 4:
            texture, sampler, coords, component = args
        else:
            raise ValueError(
                "WGSL target supports textureGather() calls with texture/coords, "
                "texture/coords/component, texture/sampler/coords, or "
                "texture/sampler/coords/component arguments; got "
                f"{len(args)} argument(s) for {function_name}"
            )
    ...
    def generate_texture_dimensions_call(self, args):
        ...
        expected_shape = self.vector_shape(self._current_expression_expected_type)
        if expected_shape is not None:
            element_type, size = expected_shape
            if element_type == "f32":
                return f"vec{size}<f32>({dimensions})"
        return dimensions
    ...
    def is_sampler_expression(self, expr):
        if self.is_sampler_type(self.expression_type(expr)):
            return True
        resource_binding = self.resource_member_binding_for_access(expr)
        return resource_binding is not None and self.is_sampler_type(
            resource_binding["member_type"]
        )

    def is_comparison_texture_expression(self, expr):
        resource_binding = self.resource_member_binding_for_access(expr)
        if resource_binding is not None:
    ...
    def require_depth_texture_operand(self, texture_expr, function_name):
        if not self.is_comparison_texture_expression(texture_expr):
            raise ValueError(
    ...
    def require_comparison_sampler_operand(self, args, function_name, *, implicit):
        ...
        if len(args) != implicit + 1:
            return
        if not self.is_comparison_sampler_expression(args[1]):
            raise ValueError(
    ...
    def generate_constructor(self, node):
        if len(node.arguments) == 1:
            ...
            if narrowed is not None:
                return narrowed
        vector_shape = self.vector_shape(node.constructor_type)
        if vector_shape is not None:
            element_type, _size = vector_shape
            args = ", ".join(
                self.generate_vector_constructor_argument(arg, element_type)
    ...
    def generic_constructor_type_name(self, name):
        parsed = self.generic_constructor_parts(name)
        if parsed is None:
            return None
        base_name, generic_args = parsed
        key = self.generic_struct_type_key(base_name, generic_args)
        specialized_name = self._generic_struct_specialized_names.get(key)
        if specialized_name is None:
            return None
        return self.type_identifier_name(specialized_name)

    def generic_constructor_parts(self, name):
        match = re.fullmatch(r"\s*([A-Za-z_][0-9A-Za-z_]*)\s*<(.+)>\s*", str(name))
        if match is None:
            return None
        base_name = match.group(1)
        if base_name not in self._generic_struct_definitions:
            return None
        generic_args = self.split_generic_constructor_arguments(match.group(2))
        if generic_args is None:
            return None
        return base_name, generic_args

    def split_generic_constructor_arguments(self, args_text):
        args = []
        depth = 0
        current = []
    ...
    def generate_type_constructor_call_arguments(self, function_name, arguments):
        ...
        if vector_shape is None:
            return self.generate_call_arguments(function_name, arguments)
        element_type, _size = vector_shape
        return ", ".join(
            self.generate_vector_constructor_argument(arg, element_type)
    ...
    def type_name_string(self, vtype, allow_storage_resources=False):
        ...
        if isinstance(vtype, NamedType):
            ...
            if vtype.generic_args:
                if vtype.name in self._generic_struct_definitions:
                    return self.generic_struct_specialized_type_name(vtype)
                raise ValueError(
                    "WGSL target does not support generic named type "
                    f"{self.generic_type_key(vtype)}; only concrete generic "
                    "struct specializations can be emitted"
                )
            if vtype.name in self._generic_struct_definitions:
                definition = self._generic_struct_definitions[vtype.name]
                expected = len(getattr(definition, "generic_params", []) or [])
                raise ValueError(
                    "WGSL target cannot use generic struct "
                    f"{vtype.name} without concrete type arguments; expected "
                    f"{expected} argument{'s' if expected != 1 else ''}"
                )
            return self.map_type_name(vtype.name)
        if isinstance(vtype, GenericType):
    ...
    def map_type_name(self, type_name):
        ...
        storage_texture_type = self.storage_texture_type(lower)
        if storage_texture_type is not None:
            return storage_texture_type
    ...
    def module_storage_access_modes(
        self, global_variables, stage_resource_parameters=()
    ):
        modes = {}
        for variable in global_variables:
    ...
    def stage_interface_array_member_info(self, struct_name, member_name):
        if struct_name not in self._stage_interface_struct_names:
            return None
        member_type = self._struct_member_types.get((struct_name, member_name))
        return self.stage_interface_array_member_info_for_type(member_type)

    def stage_interface_array_member_info_for_type(self, member_type):
        if not isinstance(member_type, ArrayType):
            return None
        size = self.resolve_integer_expression(member_type.size)
        ...
        try:
            ...
            vector_shape = self.vector_shape(element_type)
        except ValueError:
            return None
        if scalar_type not in {"f32", "i32", "u32"} and vector_shape is None:
            return None
        if vector_shape is not None and vector_shape[0] not in {"f32", "i32", "u32"}:
            return None
        return {
            "size": size,
            "element_type": element_type,
    ...
    def stage_interface_matrix_member_info(self, struct_name, member_name):
        if struct_name not in self._stage_interface_struct_names:
            return None
        member_type = self._struct_member_types.get((struct_name, member_name))
        return self.stage_interface_matrix_member_info_for_type(member_type)

    def stage_interface_matrix_member_info_for_type(self, member_type):
        if not isinstance(member_type, MatrixType):
            return None
        try:
            matrix_type_name = self.type_name_string(member_type)
            element_type_name = self.type_name_string(member_type.element_type)
        except ValueError:
            return None
        if element_type_name not in {"f32", "i32", "u32"}:
            return None
        return {
            "columns": member_type.cols,
            "rows": member_type.rows,
            "matrix_type_name": matrix_type_name,
            "column_type_name": f"vec{member_type.rows}<{element_type_name}>",
        }

    def stage_interface_array_field_name(self, struct_name, member_name, index):
        member_name = self.struct_member_identifier_name(struct_name, member_name)
        return f"{member_name}_{index}"

    def stage_interface_array_access(self, expr):
        if not isinstance(expr, ArrayAccessNode):
            return None
        ...
        return {
            **member_info,
            "struct_name": struct_name,
            "member_name": array_expr.member,
            "object_expr": array_expr.object_expr,
            "index_expr": expr.index_expr,
        }

    def stage_interface_array_access_field(self, access, index):
        object_expr = self.generate_expression(access["object_expr"])
        field_name = self.stage_interface_array_field_name(
            access["struct_name"], access["member_name"], index
        )
        return f"{object_expr}.{field_name}"

    def generate_stage_interface_array_access(self, expr):
        access = self.stage_interface_array_access(expr)
        if access is None:
            return None
        index = self.resolve_integer_expression(expr.index_expr)
        if index is not None:
            ...
            return self.stage_interface_array_access_field(access, index)

        index_expr = self.generate_expression(expr.index_expr)
        result = self.stage_interface_array_access_field(access, 0)
        for field_index in range(1, access["size"]):
            field = self.stage_interface_array_access_field(access, field_index)
            result = f"select({result}, {field}, ({index_expr} == {field_index}))"
        return result

    def stage_interface_matrix_access(self, expr):
        ...
        if struct_name is None:
            return None
        member_info = self.stage_interface_matrix_member_info(struct_name, expr.member)
        if member_info is None:
            return None
        return {
            **member_info,
    ...
    def stage_interface_matrix_access_field(self, access, index):
        object_expr = self.generate_expression(access["object_expr"])
        field_name = self.stage_interface_array_field_name(
            access["struct_name"], access["member_name"], index
        )
    ...
    def generate_stage_interface_matrix_access(self, expr):
        access = self.stage_interface_matrix_access(expr)
        if access is None:
            return None
        columns = ", ".join(
            self.stage_interface_matrix_access_field(access, index)
            for index in range(access["columns"])
        )
        return f"{access['matrix_type_name']}({columns})"
    ...
    def storage_texture_type(self, vtype, *, node=None):
        type_name = self.resource_type_name(vtype)
        if type_name is None:
            return None
        texture_kind = self.STORAGE_TEXTURE_TYPE_MAP.get(type_name)
        if texture_kind is None:
            return None
        return (
            f"{texture_kind}<"
            f"{self.storage_texture_format(type_name, node)}, "
            f"{self.storage_texture_access(node)}>"
        )

    def storage_texture_format(self, type_name, node):
        for attr in getattr(node, "attributes", []) or []:
            key = self.semantic_key(str(getattr(attr, "name", attr)))
    ...
    def is_comparison_texture_type(self, vtype):
        return self.is_depth_texture_type(vtype)

    def sampler_type_name(self, vtype):
        return self.sampler_type(vtype)

    def texture_sampler_type_name(self, vtype):
        return self.companion_sampler_type(vtype)
    ...
    def struct_type_name(self, vtype):
        ...
        if isinstance(vtype, NamedType) and not vtype.generic_args:
            return str(vtype.name)
        if (
            isinstance(vtype, NamedType)
            and vtype.generic_args
            and vtype.name in self._generic_struct_definitions
        ):
            return self.generic_struct_specialized_type_name(vtype)
        if isinstance(vtype, str):
            return vtype
    ...
    def is_bool_type(self, vtype):
        return self.scalar_type_name(vtype) == "bool"

    def is_uniform_bool_member_access(self, expr):
        if not isinstance(expr, MemberAccessNode):
            return False
        object_type = self.array_element_type(self.expression_type(expr.object_expr))
        struct_name = self.struct_type_name(object_type)
    ...
    def vector_component_count(self, vtype):
        if isinstance(vtype, VectorType):
            return vtype.size
        if isinstance(vtype, str):
            match = re.fullmatch(r"vec([234])<[^>]+>", self.map_type_name(vtype))
    ...
    def vector_type_name(self, vtype):
        if self.vector_component_count(vtype) is None:
            return None
        if isinstance(vtype, str):
            return self.map_type_name(vtype)
        return self.type_name_string(vtype)
    ...
    def stage_return_attributes(self, stage_name, function):
        ...
        if not attributes and stage_name == "vertex":
            return ()
        if not attributes and stage_name == "fragment":
            return_type = getattr(function, "return_type", None)
            if (
                self.scalar_type_name(return_type) is not None
                or self.vector_shape(return_type) is not None
            ):
                return ("@location(0)",)
    ...
    def reserve_explicit_binding_for_node(self, node):
        ...
        if register_class is not None:
            ...
            if owner is None:
                ...
                target_binding = self.next_available_binding(group, numeric_binding + 1)
