    ...
    def get_size(self) -> Optional[int]:
        ...
        if self.is_vector():
            ...
            return (
                ...
                if self.matrix_rows and self.matrix_cols
                else None
            )
        elif self.is_array():
...
class TypeSystem:
    ...
    def _initialize_builtin_types(self):
        ...
        for primitive in PrimitiveTypeKind:
            desc = TypeDescriptor(
                category=TypeCategory.PRIMITIVE, primitive_kind=primitive
            )
    ...
    def get_primitive_type(self, kind: PrimitiveTypeKind) -> TypeDescriptor:
        ...
        return TypeDescriptor(category=TypeCategory.PRIMITIVE, primitive_kind=kind)

    def get_vector_type(
        self, element_type: TypeDescriptor, size: int
    ) -> TypeDescriptor:
        ...
        return TypeDescriptor(
            category=TypeCategory.VECTOR, element_type=element_type, vector_size=size
        )

    def get_matrix_type(
        self, element_type: TypeDescriptor, rows: int, cols: int
    ) -> TypeDescriptor:
    ...
    def get_array_type(
        self, element_type: TypeDescriptor, size: Union[int, str]
    ) -> TypeDescriptor:
        """Get an array type descriptor."""
        return TypeDescriptor(
            category=TypeCategory.ARRAY, element_type=element_type, array_size=size
        )

    def get_pointer_type(
        self, pointee_type: TypeDescriptor, is_mutable: bool = True
    ) -> TypeDescriptor:
    ...
    def get_function_type(
        self, return_type: TypeDescriptor, param_types: List[TypeDescriptor]
    ) -> TypeDescriptor:
    ...
    def register_struct(self, name: str, members: Dict[str, TypeDescriptor]):
        ...
        struct_type = TypeDescriptor(category=TypeCategory.STRUCT, struct_name=name)
    ...
    def _parse_core_type(self, type_str: str) -> TypeDescriptor:
        ...
        if "*" in type_str:
            ...
            return TypeDescriptor(
                category=TypeCategory.POINTER, pointee_type=pointee_type
            )
        ...
        for pattern, (element_kind, _) in vector_patterns.items():
            ...
            if match:
                ...
                element_type = TypeDescriptor(
                    category=TypeCategory.PRIMITIVE, primitive_kind=element_kind
                )
                return TypeDescriptor(
                    category=TypeCategory.VECTOR,
                    element_type=element_type,
        ...
        if type_str in texture_types:
            return TypeDescriptor(category=TypeCategory.TEXTURE, struct_name=type_str)

        # Assume it's a user-defined struct
        return TypeDescriptor(category=TypeCategory.STRUCT, struct_name=type_str)

    def are_compatible(
        self, from_type: TypeDescriptor, to_type: TypeDescriptor
    ) -> bool:
        ...
        if from_type.category == to_type.category:
            if from_type.category == TypeCategory.PRIMITIVE:
                ...
                return (
                    from_type.vector_size == to_type.vector_size
                    and self.are_compatible(
                        from_type.element_type, to_type.element_type
                    )
                )
            elif from_type.category == TypeCategory.MATRIX:
                return (
                    from_type.matrix_rows == to_type.matrix_rows
                    and from_type.matrix_cols == to_type.matrix_cols
                    and self.are_compatible(
                        from_type.element_type, to_type.element_type
                    )
                )
    ...
    def _are_primitives_compatible(
        self, from_type: TypeDescriptor, to_type: TypeDescriptor
    ) -> bool:
    ...
    def _are_cross_category_compatible(
        self, from_type: TypeDescriptor, to_type: TypeDescriptor
    ) -> bool:
    ...
    def _find_common_type(
        ...
        if (
            type1.category == TypeCategory.PRIMITIVE
            and type2.category == TypeCategory.PRIMITIVE
            and type1.is_numeric()
            and type2.is_numeric()
        ):

            # Promote to wider type
            if type1.is_float() or type2.is_float():
                # Float promotion
                if type1.primitive_kind in {
                    PrimitiveTypeKind.FLOAT64,
                    PrimitiveTypeKind.DOUBLE,
                } or type2.primitive_kind in {
                    PrimitiveTypeKind.FLOAT64,
                    PrimitiveTypeKind.DOUBLE,
                }:
...
class LanguageTypeMapper:
    ...
    def _map_vector_type(cls, type_desc: TypeDescriptor, target_language: str) -> str:
        ...
        if type_desc.element_type and type_desc.element_type.primitive_kind:
            ...
            base_type = element_mapping.get(
                type_desc.element_type.primitive_kind, "float"
            )

            # Handle special cases
            if target_language == "opengl":
                # GLSL vector naming
                if type_desc.element_type.primitive_kind in {
                    PrimitiveTypeKind.INT32,
                    PrimitiveTypeKind.INT,
                }:
                    return f"ivec{type_desc.vector_size}"
                elif type_desc.element_type.primitive_kind in {
                    PrimitiveTypeKind.UINT32,
                    PrimitiveTypeKind.UINT,
                }:
    ...
    def _map_matrix_type(cls, type_desc: TypeDescriptor, target_language: str) -> str:
        ...
        if target_language in {"cuda", "metal", "directx", "hip", "slang"}:
            ...
            if type_desc.element_type and type_desc.element_type.primitive_kind:
                element_mapping = cls.LANGUAGE_MAPPINGS.get(target_language, {})
                base_type = element_mapping.get(
                    type_desc.element_type.primitive_kind, "f32"
                )
            ...
            if type_desc.element_type and type_desc.element_type.primitive_kind:
                element_mapping = cls.LANGUAGE_MAPPINGS.get(target_language, {})
                base_type = element_mapping.get(
                    type_desc.element_type.primitive_kind, "Float32"
                )
