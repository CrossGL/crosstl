...
class LexicalError(CrossTLError):
    ...
    def __init__(self, message: str, location: Optional[SourceLocation] = None, **kwargs):
        super().__init__(message, ErrorCode.ILLEGAL_CHARACTER, ErrorSeverity.ERROR, location, **kwargs)
...
class SyntaxError(CrossTLError):
    ...
    def __init__(self, message: str, location: Optional[SourceLocation] = None, **kwargs):
        super().__init__(message, ErrorCode.UNEXPECTED_TOKEN, ErrorSeverity.ERROR, location, **kwargs)
...
class SemanticError(CrossTLError):
    ...
    def __init__(self, message: str, location: Optional[SourceLocation] = None, **kwargs):
        super().__init__(message, ErrorCode.TYPE_MISMATCH, ErrorSeverity.ERROR, location, **kwargs)
...
class TranslationError(CrossTLError):
    ...
    def __init__(self, message: str, location: Optional[SourceLocation] = None, **kwargs):
        super().__init__(message, ErrorCode.TRANSLATION_FAILED, ErrorSeverity.ERROR, location, **kwargs)
...
class ErrorCollector:
    ...
    def add_lexical_error(self, message: str, location: Optional[SourceLocation] = None):
        """Add a lexical error."""
    ...
    def add_semantic_error(self, message: str, location: Optional[SourceLocation] = None):
        """Add a semantic error."""
        self.add_error(SemanticError(message, location))
    
    def add_translation_error(self, message: str, location: Optional[SourceLocation] = None):
...
def create_unexpected_token_error(expected: str, actual: str, location: Optional[SourceLocation] = None) -> SyntaxError:
    """Create an unexpected token error."""
    return SyntaxError(f"Expected {expected}, got {actual}", location)


def create_undefined_identifier_error(identifier: str, location: Optional[SourceLocation] = None) -> SemanticError:
    """Create an undefined identifier error."""
    return SemanticError(f"Undefined identifier '{identifier}'", location)


def create_type_mismatch_error(expected: str, actual: str, location: Optional[SourceLocation] = None) -> SemanticError:
    """Create a type mismatch error."""
    return SemanticError(f"Type mismatch: expected {expected}, got {actual}", location)


def create_unsupported_feature_error(feature: str, backend: str, location: Optional[SourceLocation] = None) -> TranslationError:
    """Create an unsupported feature error."""
    return TranslationError(f"Feature '{feature}' is not supported in {backend} backend", location)
