...
from .common.errors import (
    ErrorCollector, report_error, report_translation_error, 
    SourceLocation, ErrorCode, ErrorSeverity
)
...
class TranslationEngine:
    ...
    def translate_file(self, 
        ...
        try:
            ...
            if not os.path.exists(input_path):
                report_error(f"Input file not found: {input_path}", ErrorCode.TRANSLATION_FAILED)
                return None
            ...
            if not source_backend:
                report_error(f"Unsupported source file type: {input_path}", ErrorCode.UNSUPPORTED_FEATURE)
                return None
            
            # Get target backend
            target_backend = BackendRegistry.get_backend(target_language)
            if not target_backend:
                report_error(f"Unsupported target language: {target_language}", ErrorCode.UNSUPPORTED_FEATURE)
                return None
    ...
    def translate_code(self,
        ...
        try:
            ...
            if not source_backend:
                report_error(f"Unsupported source language: {source_language}", ErrorCode.UNSUPPORTED_FEATURE)
                return None
    ...
    def _perform_translation(self, 
        ...
        try:
            ...
            if source_backend.name != "crossgl":
                ...
                crossgl_ir = crossgl_converter.generate(ast)
    ...
    def _format_code(self, code: str, language: str) -> str:
        ...
        try:
            ...
            report_error(f"Code formatting failed: {str(e)}", ErrorCode.TRANSLATION_FAILED, ErrorSeverity.WARNING)
            return code
    ...
    def _save_output(self, code: str, output_path: str):
        ...
        try:
            ...
            report_error(f"Failed to save output file: {str(e)}", ErrorCode.TRANSLATION_FAILED)
...
def translate_file(input_path: str, 
    ...
    return engine.translate_file(input_path, target_language, output_path, format_output)


def translate_code(source_code: str,
                  source_language: str,
                  target_language: str, 
                  format_output: bool = True) -> Optional[str]:
    """
    ...
    """
    engine = get_translation_engine()
    return engine.translate_code(source_code, source_language, target_language, format_output)
