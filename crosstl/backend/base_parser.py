...
class BaseParser(ABC):
    ...
    def advance(self) -> Token:
        ...
        if self.current_index < len(self.tokens) - 1:
            ...
            self.current_token = Token(
                TokenType.EOF, "", self.current_token.line if self.current_token else 1
            )
    ...
    def parse_binary_expression(self, min_precedence: int = 0) -> ExpressionNode:
        ...
        while True:
            if not self.current_token or not self._is_binary_operator(
            ...
            next_min_prec = precedence + (
                1 if self._is_left_associative(op_token.type) else 0
            )
    ...
    def _get_precedence(self, token_type: TokenType) -> int:
        ...
        precedence_map = {
            ...
            TokenType.SHIFT_LEFT: 8,
            TokenType.SHIFT_RIGHT: 8,
            TokenType.PLUS: 9,
            TokenType.MINUS: 9,
    ...
    def parse_comma_separated_list(
        self, parse_func, terminator: TokenType
    ) -> List[Any]:
...
class StandardParser(BaseParser):
    ...
    def parse_type(self) -> str:
        ...
        if self.match(
            ...
            self.error(
                f"Expected type, got {self.current_token.type.name if self.current_token else 'EOF'}"
            )
    ...
    def is_function_declaration(self) -> bool:
        ...
        try:
            # Skip qualifiers
            while self.match(
                TokenType.CONST, TokenType.STATIC, TokenType.INLINE, TokenType.EXTERN
            ):
                self.advance()
