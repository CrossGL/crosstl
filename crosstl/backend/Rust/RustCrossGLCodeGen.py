...
class RustToCrossGLConverter:
    ...
    def generate(self, ast):
        ...
        for impl_block in ast.impl_blocks:
            ...
            for func in impl_block.functions:
                code += self.generate_function(func, indent=1, struct_name=impl_block.struct_name)
    ...
    def generate_if_statement(self, node, indent):
        ...
        if node.else_body:
            if isinstance(node.else_body, list) and len(node.else_body) == 1 and isinstance(node.else_body[0], IfNode):
                # else if
                code += " else "
    ...
    def generate_expression(self, expr):
        if isinstance(expr, str):
            ...
            elements = ", ".join(self.generate_expression(elem) for elem in expr.elements)
            return f"({elements})"
    ...
    def map_type(self, rust_type):
        ...
        if "<" in rust_type and ">" in rust_type:
            ...
            if base_type in ["Vec2", "Vec3", "Vec4"]:
                return self.type_map.get(rust_type, self.type_map.get(base_type, rust_type))
