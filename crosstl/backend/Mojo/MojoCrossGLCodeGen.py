...
class MojoToCrossGLConverter:
    ...
    def generate(self, ast):
        ...
        if hasattr(ast, 'functions'):
            ...
            for struct_node in structs:
                ...
                for member in struct_node.members:
                    semantic = self.map_semantic(member.attributes) if hasattr(member, 'attributes') else ""
        ...
        if hasattr(ast, 'functions'):
            ...
            for func in functions:
                if func.qualifier == "vertex" or self.has_vertex_attribute(func) or "vertex" in func.name:
                    code += "    // Vertex Shader\n"
                    code += "    vertex {\n"
                    code += self.generate_function(func, indent=2)
                    code += "    }\n\n"
                elif func.qualifier == "fragment" or self.has_fragment_attribute(func) or "fragment" in func.name:
                    code += "    // Fragment Shader\n" 
                    code += "    fragment {\n"
                    code += self.generate_function(func, indent=2)
                    code += "    }\n\n"
                elif func.qualifier == "compute" or self.has_compute_attribute(func) or "compute" in func.name:
                    code += "    // Compute Shader\n"
                    code += "    compute {\n"
                    code += self.generate_function(func, indent=2)
                    code += "    }\n\n"
    ...
    def generate_function(self, func, indent=1):
        ...
        code += f"{indent_str}{return_type} {func.name}({params_str}){func_attributes} {{\n"
