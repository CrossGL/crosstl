...
class HipToCrossGLConverter:
    ...
    def generic_convert(self, node: HipASTNode) -> ASTNode:
        ...
        raise NotImplementedError(f"Conversion not implemented for {type(node).__name__}")
    ...
    def convert_HipFunctionNode(self, node: HipFunctionNode) -> FunctionNode:
        ...
        is_device = '__device__' in node.qualifiers or '__host__' in node.qualifiers
        
        # Convert return type
        return_type = self.convert_type(node.return_type) if node.return_type else 'void'
    ...
    def convert_HipVariableDeclarationNode(self, node: HipVariableDeclarationNode) -> List[VariableNode]:
            ...
            if var.dimensions:
                for dim in var.dimensions:
                    if dim:
                        ...
                        var_type = f"{var_type}[{self.convert_expression_to_string(dim)}]"
                    else:
                        var_type = f"{var_type}[]"
    ...
    def convert_HipExpressionStatementNode(self, node: HipExpressionStatementNode) -> ExpressionStatementNode:
    ...
    def convert_HipFunctionCallNode(self, node: HipFunctionCallNode) -> FunctionCallNode:
    ...
    def convert_HipMemberAccessNode(self, node: HipMemberAccessNode) -> MemberAccessNode:
    ...
    def convert_HipPointerMemberAccessNode(self, node: HipPointerMemberAccessNode) -> MemberAccessNode:
        """Convert HIP pointer member access to CrossGL member access"""
        # Convert -> to . for CrossGL
        object_expr = self.convert(node.object)
        return MemberAccessNode(object_expr, node.member)
    
    def convert_HipInitializerListNode(self, node: HipInitializerListNode) -> ArrayLiteralNode:
        """Convert HIP initializer list to CrossGL array literal"""
        elements = [self.convert(elem) for elem in node.elements]
        return ArrayLiteralNode(elements)
    
    def convert_HipPreprocessorNode(self, node: HipPreprocessorNode) -> Optional[ASTNode]:
