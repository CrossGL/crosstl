...
class CudaParser:
    ...
    def parse(self):
        ...
        while self.current_token[0] != "EOF":
            ...
            elif self.current_token[0] in ["GLOBAL", "DEVICE", "HOST"] or self.peek_function():
                func = self.parse_function()
                if isinstance(func, KernelNode):
                    kernels.append(func)
                else:
                    functions.append(func)
            elif self.current_token[0] in ["CONSTANT", "SHARED"] or self.peek_variable():
    ...
    def peek_function(self):
        ...
        while (saved_index < len(self.tokens) and 
               self.tokens[saved_index][0] in ["CONST", "STATIC", "INLINE", "EXTERN"]):
            saved_index += 1
            
        # Skip type (could be multiple tokens)
        if saved_index < len(self.tokens) and self.tokens[saved_index][0] in [
            "VOID", "INT", "FLOAT", "DOUBLE", "CHAR", "SHORT", "LONG", "BOOL",
            "IDENTIFIER", "UNSIGNED", "SIGNED"
        ]:
            saved_index += 1
            
        # Check for function name followed by parenthesis
        if (saved_index < len(self.tokens) - 1 and 
            self.tokens[saved_index][0] == "IDENTIFIER" and
            self.tokens[saved_index + 1][0] == "LPAREN"):
            return True
    ...
    def peek_variable(self):
        ...
        # Skip qualifiers
        while (saved_index < len(self.tokens) and 
               self.tokens[saved_index][0] in ["CONST", "STATIC", "EXTERN"]):
            saved_index += 1
            
        # Check for type
        if saved_index < len(self.tokens) and self.tokens[saved_index][0] in [
            "INT", "FLOAT", "DOUBLE", "CHAR", "SHORT", "LONG", "BOOL",
            "IDENTIFIER", "UNSIGNED", "SIGNED", "FLOAT2", "FLOAT3", "FLOAT4"
        ]:
            saved_index += 1
            # Check for identifier
            if (saved_index < len(self.tokens) and 
                self.tokens[saved_index][0] == "IDENTIFIER"):
                saved_index += 1
                # Check for semicolon or assignment
                if (saved_index < len(self.tokens) and 
                    self.tokens[saved_index][0] in ["SEMICOLON", "ASSIGN"]):
                    return True
    ...
    def parse_type(self):
        ...
        if self.current_token[0] in [
            ...
            "DOUBLE2", "DOUBLE3", "DOUBLE4", "SIZE_T"
        ]:
            type_parts.append(self.current_token[1])
            self.eat(self.current_token[0])
    ...
    def parse_variable_declaration(self):
        ...
        return VariableNode(vtype, name, value)
    ...
    def is_variable_declaration(self):
        ...
        if self.current_token[0] in [
            ...
            "FLOAT2", "FLOAT3", "FLOAT4", "IDENTIFIER"
        ]:
            # Look ahead for identifier
            if (self.current_index + 1 < len(self.tokens) and 
                self.tokens[self.current_index + 1][0] == "IDENTIFIER"):
                return True
        return False
    ...
    def parse_primary_expression(self):
        ...
        if self.current_token[0] == "NUMBER":
            ...
            self.eat("IDENTIFIER")
            return name
        elif self.current_token[0] in ["THREADIDX", "BLOCKIDX", "GRIDDIM", "BLOCKDIM", "WARPSIZE"]:
            ...
            raise SyntaxError(f"Unexpected token in primary expression: {self.current_token}") 