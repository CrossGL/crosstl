...
TOKENS = tuple(
    [
        ...
        (
            "NUMBER",
            r"(?:0[xX](?:(?:[0-9a-fA-F](?:'?[0-9a-fA-F])*)?\.(?:[0-9a-fA-F](?:'?[0-9a-fA-F])*)|(?:[0-9a-fA-F](?:'?[0-9a-fA-F])*)\.?)[pP][+-]?\d(?:'?\d)*[fFlL]?|0[xX][0-9a-fA-F](?:'?[0-9a-fA-F])*|0[bB][01](?:'?[01])*|\d(?:'?\d)*\.\d(?:'?\d)*|\d(?:'?\d)*\.|\.\d(?:'?\d)*|\d(?:'?\d)*)(?:[eE][+-]?\d(?:'?\d)*)?[fFdDlLuU]*",
        ),
        (
            "STRING",
            r'(?:u8|u|U|L)?R"([^\s()\\]{0,16})\([\s\S]*?\)\1"(?:[a-zA-Z_][a-zA-Z0-9_]*)?',
        ),
