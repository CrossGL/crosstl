...
class CodeFormatter:
    ...
    def __init__(self, clang_format_path=None, spirv_tools_path=None):
        ...
        if not self.has_clang:
            logger.warning("clang-format not found. Install it for C-like shader formatting.")
        if not self.has_spirv_tools:
            logger.warning("SPIRV-Tools not found. Install them for proper SPIR-V handling.")
    ...
    def _format_spirv(self, code):
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(suffix=".spvasm", mode='w+', delete=False) as tmp_in:
    ...
    def validate_spirv(self, code):
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(suffix=".spvasm", mode='w+', delete=False) as tmp:
                tmp_path = tmp.name
                tmp.write(code)
