...
_OPENCL_TARGET_SCALAR_TYPE_ALIASES = {
    "f32": "float",
    "f64": "double",
    "i32": "int",
    "u32": "uint",
}
_OPENCL_STAGE_CONTROL_ATTRIBUTES = {"compute", "workgroup_size", "local_size"}
_OPENCL_SYNTHETIC_BUILTIN_ALIASES = {
...
def _clone_opencl_target_type(type_node):
    if isinstance(type_node, PrimitiveType):
        return PrimitiveType(
            _OPENCL_TARGET_SCALAR_TYPE_ALIASES.get(type_node.name, type_node.name),
            size_bits=type_node.size_bits,
        )
    if isinstance(type_node, VectorType):
        return VectorType(
            _clone_opencl_target_type(type_node.element_type),
            type_node.size,
        )
    ...
    if isinstance(type_node, NamedType):
        return NamedType(
            type_node.name,
            [_clone_opencl_target_type(arg) for arg in type_node.generic_args],
        )
    return type_node
...
def _opencl_parameter_binding(parameter):
...
def _opencl_stage_execution_config(function):
...
def _opencl_target_function_attributes(function):
    return [
        attribute
        for attribute in getattr(function, "attributes", []) or []
        if str(getattr(attribute, "name", "")).lower()
        not in _OPENCL_STAGE_CONTROL_ATTRIBUTES
    ]


def _is_opencl_synthetic_builtin_alias(statement):
    if not isinstance(statement, VariableNode):
        return False
    expected_builtin = _OPENCL_SYNTHETIC_BUILTIN_ALIASES.get(statement.name)
...
def _strip_opencl_synthetic_builtin_aliases(function):
    body = getattr(function, "body", None)
    statements = getattr(body, "statements", None)
    if not isinstance(statements, list):
        return
    while statements and _is_opencl_synthetic_builtin_alias(statements[0]):
        statements.pop(0)


def _opencl_structured_buffer_type(parameter, writable=True):
    ...
    return NamedType(buffer_name, [_clone_opencl_target_type(element_type)])


def _opencl_stage_local_variable(parameter):
    param_type = _clone_opencl_target_type(getattr(parameter, "param_type", None))
...
def _opencl_target_kernel_cbuffer(function_name, scalar_parameters, binding):
    members = [
        StructMemberNode(
            parameter.name,
            _clone_opencl_target_type(getattr(parameter, "param_type", None)),
        )
...
def _opencl_resource_qualifiers(parameter):
    return {
        str(qualifier).lower()
...
def _is_opencl_target_compute_function(function):
...
def _normalize_opencl_intermediate_for_target(cgl_ast):
    ...
    for function in functions:
        if _is_opencl_target_compute_function(function):
    ...
    for function in target_functions:
        ...
        for parameter in getattr(function, "parameters", []) or []:
            qualifiers = _opencl_resource_qualifiers(parameter)
            if "workgroup" in qualifiers:
                local_variables.append(_opencl_stage_local_variable(parameter))
                continue
            if "storage" in qualifiers:
                binding = _opencl_parameter_binding(parameter)
                if binding is None:
                ...
                global_variables.append(
                    VariableNode(
                        parameter.name,
                        _opencl_structured_buffer_type(parameter, writable=True),
                        attributes=[_binding_attribute(binding)],
                    )
                )
                continue
        ...
        function.parameters = []
        function.attributes = _opencl_target_function_attributes(function)
        _strip_opencl_synthetic_builtin_aliases(function)
        cgl_ast.stages.append(
            ...
            StageNode(
                ...
                execution_config=_opencl_stage_execution_config(function),
            ),
        )
    ...
    if source_spec.name == "cgl":
        ...
        if normalized_backend in ["cgl", "crossgl"]:
            ...
            if source_spec.name == "opencl" and normalized_backend != "webgl":
                cgl_ast = _normalize_opencl_intermediate_for_target(cgl_ast)
            codegen = get_codegen(normalized_backend)
