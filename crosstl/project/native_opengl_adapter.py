...
def generate_opengl_native_loader_adapter() -> str:
    ...
    return textwrap.dedent(r"""
         ...
         * set-zero shader-storage buffers, and set-zero uniform buffers.
         * Texture, image, sampler, scalar-uniform, specialization-constant,
         * shared-allocation, and nonzero descriptor-set contracts return an
        ...
        static inline const char *crosstl_opengl43_platform_requirements(void) {
            return "caller-owned current desktop OpenGL 4.3+ context; "
                   ...
                   "execution on the context-owning thread";
        }
        ...
        #define CROSSTL_OPENGL43_REQUIRE(member, name)                            \
            ...
            CROSSTL_OPENGL43_REQUIRE(shader_source, "glShaderSource")
            CROSSTL_OPENGL43_REQUIRE(compile_shader, "glCompileShader")
        ...
        static inline int32_t crosstl_opengl43_check_error(
            ...
            return crosstl_opengl43_report(
                context, CROSSTL_OPENGL43_STATUS_OPENGL_ERROR, phase, message);
        }
        ...
        static inline std::string crosstl_opengl43_artifact_path(
            const CrossTLOpenGL43Context *context,
            const char *package_path) {
            if (context->artifact_root == NULL ||
                context->artifact_root[0] == '\0' ||
                crosstl_opengl43_is_absolute_path(package_path)) {
                return std::string(package_path);
            }
            std::string path(context->artifact_root);
        ...
        static inline int32_t crosstl_opengl43_load_artifact(
            ...
            if (!crosstl_native_loader_strings_equal(
                ...
                return crosstl_opengl43_report(
                    ...
                    "load-artifact",
                    "OpenGL 4.3 adapter artifacts must use GLSL source format.");
            }
            if (unit->artifact_path == NULL || unit->artifact_path[0] == '\0') {
                return crosstl_opengl43_report(
                    ...
                    CROSSTL_OPENGL43_STATUS_INVALID_ARGUMENT,
                    "load-artifact",
                    "The GLSL artifact path is missing.");
            }
            try {
                const std::string path =
                    crosstl_opengl43_artifact_path(context, unit->artifact_path);
                ...
                if (!stream) {
                    return crosstl_opengl43_report(
                        ...
                        "load-artifact",
                        "The GLSL artifact could not be opened.");
                }
                ...
                if (size != unit->artifact_size_bytes) {
                    return crosstl_opengl43_report(
                        ...
                        "load-artifact",
                        "The GLSL artifact size does not match its descriptor.");
                }
                if (size >
                    static_cast<uint64_t>(
                        std::numeric_limits<CrossTLOpenGL43Int>::max())) {
                    return crosstl_opengl43_report(
                        context,
                        CROSSTL_OPENGL43_STATUS_ARTIFACT_SIZE_UNSUPPORTED,
                        "load-artifact",
                        "The GLSL artifact exceeds the OpenGL source length limit.");
                }
                ...
                if (artifact == NULL) {
                    return crosstl_opengl43_report(
                        ...
                        "load-artifact",
                        "The GLSL artifact allocation failed.");
                }
                try {
                    artifact->source.resize(static_cast<size_t>(size));
                } catch (...) {
                    delete artifact;
                    return crosstl_opengl43_report(
                        context,
                        CROSSTL_OPENGL43_STATUS_OUT_OF_MEMORY,
                        "load-artifact",
                        "The GLSL source allocation failed.");
                }
                stream.seekg(0, std::ios::beg);
                stream.read(
                    &artifact->source[0],
                    static_cast<std::streamsize>(artifact->source.size()));
                if (!stream) {
                    delete artifact;
                    return crosstl_opengl43_report(
                        context,
                        CROSSTL_OPENGL43_STATUS_ARTIFACT_READ_FAILED,
                        "load-artifact",
                        "The complete GLSL artifact could not be read.");
                }
                *artifact_out = artifact;
                ...
                return crosstl_opengl43_report(
                    ...
                    "load-artifact",
                    "An exception occurred while loading the GLSL artifact.");
            }
        }
        ...
        static inline int32_t crosstl_opengl43_create_pipeline(
            ...
            const CrossTLOpenGL43Char *source = artifact->source.c_str();
            const CrossTLOpenGL43Int length =
                static_cast<CrossTLOpenGL43Int>(artifact->source.size());
            context->functions.shader_source(shader, 1, &source, &length);
            context->functions.compile_shader(shader);
            status = crosstl_opengl43_check_error(
                context, "create-pipeline", "GLSL shader compilation");
            if (status != CROSSTL_OPENGL43_STATUS_OK) {
            ...
            status = crosstl_opengl43_check_error(
                context, "create-pipeline", "GLSL program linking");
            if (status != CROSSTL_OPENGL43_STATUS_OK) {
        ...
        static inline int32_t crosstl_opengl43_apply_specialization(
            ...
            return crosstl_opengl43_report(
                ...
                "GLSL source specialization constants require a separately "
                "compiled and specialized SPIR-V execution contract.");
        }
