...
- ``artifacts``: attempted outputs with stable repository-relative POSIX source
  ...
  whose source and generated files match the anchors.
  ...
  pipeline currently emits file-granularity source maps; line, statement, and
  token mappings are accepted for reports produced by downstream tooling but
  are not generated by the translator project pipeline yet.
