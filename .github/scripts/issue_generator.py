...
class IssueGenerator:
        ...
        if cache_file.exists():
            try:
                ...
                if (datetime.now() - cache_time).days < 1:
                    ...
                    return {
                        issue["title"]: issue for issue in cache_data.get("issues", [])
                    }
    ...
    def _create_issue(
        ...
        for attempt in range(max_retries):
            try:
                ...
                if e.status == 403 and attempt < max_retries - 1:
                    ...
                    logger.warning(
                        f"Rate limit hit, waiting 60 seconds... ({attempt+1}/{max_retries})"
                    )
                    time.sleep(60)
                else:
        ...
        raise RuntimeError(
            f"Failed to create issue after {max_retries} attempts: {title}"
        )

    def _format_feature_documentation(
        self, feature: Dict[str, Any], language: str
    ) -> str:
        """Format feature documentation for inclusion in issue body"""
        feature_type = (
            "function"
            if "signature" in feature
            else "operator" if "symbol" in feature else "type"
        )
    ...
    def _generate_task_description(
        self, feature: Dict[str, Any], language: str, feature_type: str
    ) -> str:
...
"""
    ...
    def generate_parent_issues(
        self, language: str, analysis_data: Dict[str, Any]
    ) -> Dict[str, Issue]:
        ...
        for category, category_info in CATEGORIES.items():
            ...
            missing_count = sum(
                ...
                if self.analyzer.get_implementation_status(
            ...
            if missing_count == 0:
                logger.info(
                    f"No missing {category} for {language}, skipping parent issue"
                )
                continue
...
"""
    ...
    def generate_feature_issues(
        self, language: str, parent_issues: Dict[str, Issue]
    ) -> None:
        ...
        for category, category_info in CATEGORIES.items():
            ...
            for feature in analysis_data[category]:
                ...
                status = self.analyzer.get_implementation_status(
                    category, feature_name, language
                )
                ...
                title = (
                    f"[{language.upper()}] Implement {category[:-1]} '{feature_name}'"
                )
                labels = category_info["labels"].copy()
                labels.append(language)
                ...
                try:
                    ...
                    issue_references.append(
                        f"- [ ] {feature_name} (issue creation failed)"
                    )
            ...
            if issue_references:
                ...
                logger.info(
                    f"Updated parent issue #{parent_issue.number} with {len(issue_references)} child issues"
                )
