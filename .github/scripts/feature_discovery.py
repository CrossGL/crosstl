...
FEATURE_TYPES = {
    ...
    "types": (
        r"(?:visit_TypeDecl|process_type)\(.*?['\"](\w+)['\"]"  # Match type declarations
    ),
}
...
def get_features_for_backend(backend):
    ...
    features = {"functions": set(), "operators": set(), "types": set()}

    # Check the main codegen file
    codegen_file = CODEGEN_FILES.get(backend)
    if codegen_file and os.path.exists(codegen_file):
        for feature_type in FEATURE_TYPES:
            features[feature_type].update(
                get_features_from_file(codegen_file, feature_type)
            )
        ...
        for root, _, files in os.walk(backend_dir):
            for file in files:
                if file.endswith(".py"):
                    ...
                    for feature_type in FEATURE_TYPES:
                        features[feature_type].update(
                            get_features_from_file(file_path, feature_type)
                        )
...
def find_missing_features():
    ...
    all_features = {
        backend: get_features_for_backend(backend) for backend in REFERENCE_BACKENDS
    }
    ...
    all_possible_features = {
        feature_type: (
            set().union(
                *(
                    backend_features[feature_type]
                    for backend_features in all_features.values()
                )
            )
        )
        for feature_type in FEATURE_TYPES
    ...
    for backend in REFERENCE_BACKENDS:
        missing_features[backend] = {
            feature_type: (
                all_possible_features[feature_type]
                - all_features[backend][feature_type]
            )
            for feature_type in FEATURE_TYPES
        }
            ...
            if codegen_file and os.path.exists(codegen_file):
                ...
                if feature_type == "functions":
                    ...
                    for match in re.finditer(
                        method_pattern, content, re.DOTALL | re.MULTILINE
                    ):
                        if match.group(
                            1
                        ) == feature or f"visit_{feature}" == match.group(1):
        ...
        for feature_type in FEATURE_TYPES:
            ...
            if missing:
                ...
                for feature in sorted(missing):
                    ...
                    feature_issue_title = (
                        f"[{backend.upper()}] Implement {feature_type[:-1]} '{feature}'"
                    )
                    existing_issue = None
                    for issue in repo.get_issues(state="open"):
...
"""
                        ...
                        backend_sections[
                            backend
                        ] += f"- [ ] [{feature}](#{existing_issue.number})\n"
