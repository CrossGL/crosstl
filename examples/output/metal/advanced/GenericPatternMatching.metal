...
__attribute__((unused)) constant int MathError_InvalidInput = 1;
__attribute__((unused)) constant int MathError_Overflow = 2;
__attribute__((unused)) constant int MathError_Underflow = 3;
__attribute__((unused)) constant int VectorOp_Add = 0;
__attribute__((unused)) constant int VectorOp_Multiply = 1;
__attribute__((unused)) constant int VectorOp_Cross = 2;
__attribute__((unused)) constant int VectorOp_Normalize = 3;
__attribute__((unused)) constant int ShaderError_MathError = 0;
__attribute__((unused)) constant int ShaderError_TextureError = 1;
__attribute__((unused)) constant int ShaderError_BufferError = 2;
__attribute__((unused)) constant int ShaderError_InvalidState = 3;
__attribute__((unused)) constant int LightingModel_Phong = 0;
__attribute__((unused)) constant int LightingModel_PBR = 1;
__attribute__((unused)) constant int LightingModel_Toon = 2;
__attribute__((unused)) constant int RenderCommand_Draw = 0;
__attribute__((unused)) constant int RenderCommand_Clear = 1;
__attribute__((unused)) constant int RenderCommand_SetState = 2;
__attribute__((unused)) constant int RenderOutput_Success = 0;
__attribute__((unused)) constant int RenderOutput_Clear = 1;
__attribute__((unused)) constant int RenderOutput_StateSet = 2;

__attribute__((unused)) constant int Option_Some = 0;
__attribute__((unused)) constant int Option_None = 1;
__attribute__((unused)) constant int Result_Ok = 0;
__attribute__((unused)) constant int Result_Err = 1;
...
LightingModel LightingModel_Phong_make(float3 payload0, float3 payload1,
                                       float3 payload2, float payload3) {
...
LightingModel LightingModel_PBR_make(float3 payload0, float payload1,
...
LightingModel LightingModel_Toon_make(float3 payload0, int payload1,
                                      float payload2) {
  LightingModel result;
...
RenderCommand RenderCommand_Clear_make(float4 payload0, float payload1) {
  ...
  result.material = Option_Material();
  result.transform = float4x4(0);
  result.lighting = LightingEnvironment();
  result.state = RenderState_float();
...
RenderOutput RenderOutput_Clear_make(float4 payload0, float payload1) {
  ...
  result.Success_0 = RenderedFrame();
  result.color = payload0;
  result.depth = payload1;
  return result;
}
...
Result_RenderOutput_ShaderError
Result_RenderOutput_ShaderError_Ok_make(RenderOutput payload0) {
  Result_RenderOutput_ShaderError result;
  result.variant = Result_Ok;
  result.Err_0 = ShaderError();
  result.Ok_0 = payload0;
  return result;
}

Result_RenderOutput_ShaderError
Result_RenderOutput_ShaderError_Err_make(ShaderError payload0) {
  Result_RenderOutput_ShaderError result;
  result.variant = Result_Err;
  result.Ok_0 = RenderOutput();
  result.Err_0 = payload0;
  return result;
}

Result_RenderedFrame_MathError
Result_RenderedFrame_MathError_Ok_make(RenderedFrame payload0) {
...
Result_RenderedFrame_MathError
Result_RenderedFrame_MathError_Err_make(int payload0) {
  Result_RenderedFrame_MathError result;
  result.variant = Result_Err;
  result.Ok_0 = RenderedFrame();
  result.Err_0 = payload0;
  return result;
}
...
Result_Vec3_float_MathError
Result_Vec3_float_MathError_Ok_make(Vec3_float payload0) {
...
Result_float_MathError safe_divide_float(float a, float b) {
  ...
  return Result_float_MathError() /* fallback for unmatched generated control
                                     flow */
      ;
}
...
Result_Vec3_float_MathError vector_operation_float(Vec3_float v1, Vec3_float v2,
  ...
  if (op == VectorOp_Add) {
    ...
    Vec3_float cross_result =
        Vec3_float{(v1.y * v2.z) - (v1.z * v2.y), (v1.z * v2.x) - (v1.x * v2.z),
                   (v1.x * v2.y) - (v1.y * v2.x)};
    ...
    Result_float_MathError __crossgl_match_subject_0 =
        safe_divide_float(1.0, sqrt(mag_squared));
    if (__crossgl_match_subject_0.variant == Result_Ok) {
      __attribute__((unused)) float inv_mag = __crossgl_match_subject_0.Ok_0;
      return Result_Vec3_float_MathError_Ok_make(
          Vec3_float{(v1.x * inv_mag), (v1.y * inv_mag), (v1.z * inv_mag)});
    } else if (__crossgl_match_subject_0.variant == Result_Err) {
      __attribute__((unused)) int e = __crossgl_match_subject_0.Err_0;
      return Result_Vec3_float_MathError_Err_make(e);
    }
  }
  return Result_Vec3_float_MathError() /* fallback for unmatched generated
                                          control flow */
      ;
}

float3 process_lighting_model(LightingModel model, float3 light_dir,
                              float3 view_dir, float3 normal) {
  if (model.variant == LightingModel_Phong) {
    __attribute__((unused)) float3 ambient = model.ambient;
    __attribute__((unused)) float3 diffuse = model.diffuse;
    __attribute__((unused)) float3 specular = model.specular;
    __attribute__((unused)) float shininess = model.shininess;
    float n_dot_l = max(0.0, dot(normal, light_dir));
    float3 reflect_dir = reflect(-light_dir, normal);
    float r_dot_v = max(0.0, dot(reflect_dir, view_dir));
    return ambient + diffuse * float3(n_dot_l) +
           specular * pow(r_dot_v, shininess);
  } else if (model.variant == LightingModel_PBR) {
    __attribute__((unused)) float3 albedo = model.albedo;
    __attribute__((unused)) float metallic = model.metallic;
    __attribute__((unused)) float roughness = model.roughness;
    __attribute__((unused)) float ao = model.ao;
    ...
    float3 fresnel = f0 + float3(1.0) - f0 * pow(1.0 - n_dot_v, 5.0);
    float alpha = roughness * roughness;
    float distribution =
        alpha / 3.14159 * pow(n_dot_h * n_dot_h * alpha - 1.0 + 1.0, 2.0);
    float geometry =
        n_dot_l * n_dot_v / n_dot_l + n_dot_v - n_dot_l * n_dot_v + 0.001;
    float3 brdf = float3(distribution * geometry) * fresnel /
                  float3(4.0 * n_dot_l * n_dot_v + 0.001);
    float3 diffuse_contribution = float3(1.0) - fresnel *
                                                    float3(1.0 - metallic) *
                                                    albedo / float3(3.14159);
    return diffuse_contribution + brdf * float3(n_dot_l) * float3(ao);
  } else if (model.variant == LightingModel_Toon) {
    __attribute__((unused)) float3 base_color = model.base_color;
    __attribute__((unused)) int levels = model.levels;
    __attribute__((unused)) float smoothing = model.smoothing;
    if (levels > 0) {
      float n_dot_l = dot(normal, light_dir);
      float toon_level = floor(n_dot_l * float(levels)) / float(levels);
      float smooth_factor =
          smoothstep(toon_level - smoothing, toon_level + smoothing, n_dot_l);
      return base_color * float3(smooth_factor);
    } else {
      if (model.variant == LightingModel_Toon) {
        __attribute__((unused)) float3 base_color = model.base_color;
        return base_color * float3(0.5);
      }
    }
  } else {
    if (model.variant == LightingModel_Toon) {
  ...
  return float3(0) /* fallback for unmatched generated control flow */;
}
...
Result_int_ShaderError validate_material(Material material) {
  ...
  return Result_int_ShaderError() /* fallback for unmatched generated control
                                     flow */
      ;
}

Geometry transform_geometry(Geometry shape, float4x4 transform) {
  float4 transformed_center = transform * float4(shape.center, 1.0);
  return Geometry{transformed_center.xyz, shape.normal};
  return Geometry() /* fallback for unmatched generated control flow */;
}
...
Result_RenderedFrame_MathError apply_lighting(Geometry shape, Material material,
  ...
  float light_strength =
      ...
      lighting.intensity;
  float3 shaded_color = material.albedo * float3(light_strength);
  return Result_RenderedFrame_MathError_Ok_make(
      RenderedFrame{float4(shaded_color, 1.0), 1.0});
  return Result_RenderedFrame_MathError() /* fallback for unmatched generated
                                             control flow */
      ;
}
...
bool validate_state(RenderState_float state) {
  if (state.material_id.variant == Option_Some) {
    __attribute__((unused)) int id = state.material_id.Some_0;
    return id >= 0;
  } else if (state.material_id.variant == Option_None) {
    return true;
  }
  return false /* fallback for unmatched generated control flow */;
}
...
Result_RenderOutput_ShaderError process_render_command(RenderCommand command) {
  if (((command.variant == RenderCommand_Draw)) &&
    ...
    __attribute__((unused)) Geometry geometry = command.geometry;
    __attribute__((unused)) Material material = command.material.Some_0;
    __attribute__((unused)) float4x4 transform = command.transform;
    __attribute__((unused)) LightingEnvironment lighting = command.lighting;
    Result_int_ShaderError __crossgl_match_subject_1 =
    ...
    if (__crossgl_match_subject_1.variant == Result_Ok) {
      ...
      if (lit_result.variant == Result_Ok) {
        __attribute__((unused)) RenderedFrame output = lit_result.Ok_0;
        return Result_RenderOutput_ShaderError_Ok_make(
            RenderOutput_Success_make(output));
      } else if (lit_result.variant == Result_Err) {
        __attribute__((unused)) int lighting_error = lit_result.Err_0;
      ...
      __attribute__((unused)) ShaderError validation_error =
          __crossgl_match_subject_1.Err_0;
      return Result_RenderOutput_ShaderError_Err_make(
    ...
    __attribute__((unused)) float4 color = command.color;
    __attribute__((unused)) float depth = command.depth;
    return Result_RenderOutput_ShaderError_Ok_make(
        RenderOutput_Clear_make(color, depth));
  } else if (command.variant == RenderCommand_SetState) {
    __attribute__((unused)) RenderState_float state = command.state;
  ...
  return Result_RenderOutput_ShaderError() /* fallback for unmatched generated
                                              control flow */
      ;
}
...
vertex VertexOutput vertex_main(VertexInput input [[stage_in]],
                                ...
                                constant float3x3 &normal_matrix [[buffer(2)]],
                                constant LightingModel &lighting_model
                                [[buffer(3)]],
                                constant float3 &light_direction [[buffer(4)]],
                                constant float3 &camera_position [[buffer(5)]],
                                texture2d<float> main_texture [[texture(0)]]) {
  float4 world_pos = model_matrix * float4(input.position, 1.0);
  float4 clip_pos = mvp_matrix * float4(input.position, 1.0);
  float3 world_normal = normalize(normal_matrix * input.normal);
  ...
  if (__crossgl_match_subject_3.variant == Result_Ok) {
    __attribute__((unused)) Vec3_float normalized =
        __crossgl_match_subject_3.Ok_0;
    ...
    processed_normal = float3(0) /* fallback for generated Metal: no wildcard
                                    arm handles remaining cases */
        ;
  }
  return VertexOutput{clip_pos, world_pos.xyz, processed_normal, input.uv,
                      input.color};
  return VertexOutput() /* fallback for unmatched generated control flow */;
}
...
fragment float4 fragment_main(FragmentInput input [[stage_in]],
  ...
  float4 tex_color = main_texture.sample(
      sampler(mag_filter::linear, min_filter::linear), input.uv);
  float3 lighting_contribution =
      process_lighting_model(lighting_model, light_dir, view_dir, normal);
  ...
  return float4(0) /* fallback for unmatched generated control flow */;
}
