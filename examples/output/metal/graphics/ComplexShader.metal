...
float fbm(VectorType(element_type = PrimitiveType(name = float,
                                                  size_bits = None),
                     size = 3) p [[stage_in]],
          int octaves [[stage_in]], float lacunarity [[stage_in]],
          float gain [[stage_in]]) {
  float sum = 0.0;
  float amplitude = 1.0;
  float frequency = 1.0;
...
float4 samplePlanarProjection(
    texture2d<float> tex [[stage_in]],
    VectorType(element_type = PrimitiveType(name = float, size_bits = None),
               size = 3) worldPos [[stage_in]],
    VectorType(element_type = PrimitiveType(name = float, size_bits = None),
               size = 3) normal [[stage_in]]) {
...
vertex VertexOutput vertex_main(VertexInput input [[stage_in]]) {
  ...
  float3x3 normalMatrix = mat3(transpose(inverse(modelMatrix)));
  ...
  float3x3 TBN = mat3(worldTangent, worldBitangent, worldNormal);
  float displacement =
  ...
  if (input.materialIndex < globals.scene.activeLightCount) {
    ...
    for (int i = 0; i < 4; ++i) {
      ...
      output.color.rgb += light.color * lightIntensity *
                          max(0.0, dot(worldNormal, lightDir)) * 0.025;
    }
  } else {
...
float shadowCalculation(
    VectorType(element_type = PrimitiveType(name = float, size_bits = None),
               size = 4) fragPosLightSpace [[stage_in]],
    int iteration [[stage_in]]) {}
...
kernel void kernel_main() {
  ...
  for (int i = 0; i < 32; ++i) {
    ...
    totalWeight += weight;
    direction = mat2(cos(t * 3.0), -sin(t * 3.0), sin(t * 3.0), cos(t * 3.0)) *
                direction;
  }
