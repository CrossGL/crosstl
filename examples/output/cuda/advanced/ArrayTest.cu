...
// binding_source=automatic Constant buffer: TestBuffer
__constant__ float values[4];
__constant__ float3 colors[2];

__device__ VertexOutput vertex_main(VertexInput input) {
  VertexOutput output;
  output.uv = input.texCoord;
  float scale = (values[0] + values[1]);
  float3 position = cgl_float3_mul_scalar(input.position, scale);
  output.position = make_float4(position.x, position.y, position.z, 1.0);
  return output;
}

__device__ FragmentOutput fragment_main(FragmentInput input) {
  FragmentOutput output;
  float3 color = colors[0];
  if ((input.uv.x > 0.5)) {
    color = colors[1];
  }
  output.color = make_float4(color.x, color.y, color.z, 1.0);
  return output;
}
