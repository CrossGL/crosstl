...
__device__ VertexOutput vertex_main(VertexInput input) {
  VertexOutput output;
  output.uv = cgl_float2_mul_scalar(
      make_float2(input.position.x, input.position.y), 10.0);
  output.position =
...
__device__ float perlinNoise(float2 p) {
  return cgl_fract_float(
      (sinf(cgl_float2_dot(p, make_float2(12.9898, 78.233))) * 43758.5453));
}
...
__device__ FragmentOutput fragment_main(FragmentInput input) {
  ...
  float3 color = make_float3((height / 10.0), (1.0 - (height / 10.0)), 0.0);
