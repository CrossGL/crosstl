...
layout(std430, binding = 0) buffer ParticleBuffer { Particle particles[4096]; }
particle_buffer;
layout(std430, binding = 1) buffer AtomicCounters {
  int collision_count;
  int active_count;
  int spawn_count;
}
...
vec3 random3(vec3 seed) {
  vec3 p = vec3(dot(seed, vec3(127.1, 311.7, 74.7)),
...
vec3 resolveCollision(vec3 pos1, vec3 vel1, vec3 pos2, vec3 vel2, float mass1,
                      float mass2) {
...
void main() {
  ...
  particle.color.rgb = mix(vec3(0.2, 0.4, 1.0), vec3(1.0, 0.4, 0.2),
...
void compute_main() {
  ...
  for (int i = 0; (i < physics.max_particles); (++i)) {
    if ((!particle_buffer.particles[i].active_)) {
      Particle new_particle;
      vec3 random_offset =
          random3(vec3(i, sim_state.frame_number, sim_state.total_time));
