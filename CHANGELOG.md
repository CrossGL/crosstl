...
- Entry-scoped Metal compute artifacts with reachable helper/declaration/struct pruning, structured fail-closed selection diagnostics, bounded resource reflection, exact kernel identity, and host-owned workgroup-rule metadata; all 183 current-pinned scalar MLX `v_` unary entries (`N=1`) across 37 operators and 20 input/output type pairs round-trip to deterministic artifacts accepted by the native macOS Metal compiler.
