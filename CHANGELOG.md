...
- DirectX native 16-bit Metal `as_type` lowering now uses exact `asfloat16`/`asint16`/`asuint16` reinterpretation for matching scalar/vector shapes instead of numeric casts, preventing current-pinned MXFP4 values from collapsing to signed zero on WARP.
