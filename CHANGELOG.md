...
- DirectX native 16-bit Metal `as_type` lowering uses exact `asfloat16`/`asint16`/`asuint16` reinterpretation for matching scalar/vector shapes instead of numeric casts; mixed binary16/float arithmetic reconstructs float32 values with integer IEEE-754 masks before the operation and rounds once back to binary16, avoiding WARP's signed-zero collapse for native half conversion paths in current-pinned MXFP4.
