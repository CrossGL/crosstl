...
- Complete current-pinned MLX scalar-scalar binary Metal round trips for all 238 ``ss_`` entries across 24 operators and 25 input/output type pairs, with a hash-pinned artifact/ABI contract, strict warning-fatal native compilation, three-way required macOS CI sharding, and explicit exclusion of the remaining 3,884 non-scalar entries and numerical execution.
