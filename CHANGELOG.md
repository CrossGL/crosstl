...
- DirectX native 16-bit Metal `as_type` lowering uses exact `asfloat16`/`asint16`/`asuint16` reinterpretation by default; an explicit source-scoped widening mode reconstructs binary16 payloads with integer IEEE-754 masks and keeps logical `float16_t` arithmetic, sign application, returns, and consumers in float32, removing every native-half operation that collapsed current-pinned MXFP4 values to signed zero on WARP.
