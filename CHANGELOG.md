...
- DirectX native 16-bit Metal `as_type` lowering uses exact `asfloat16`/`asint16`/`asuint16` reinterpretation for matching scalar/vector shapes instead of numeric casts; mixed binary16/float compound arithmetic decodes payloads with `f16tof32` before the float32 operation and rounds once back to binary16, preventing current-pinned MXFP4 values from collapsing to signed zero on WARP.
