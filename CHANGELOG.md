...
- Entry-scoped Metal compute artifacts with reachable helper/declaration/struct pruning, structured fail-closed selection diagnostics, bounded resource reflection, exact kernel identity, and host-owned workgroup-rule metadata; all 30 current-pinned MLX `v_<Op>float32float32` unary entries round-trip to deterministic artifacts accepted by the native macOS Metal compiler, including the 1,015-byte Square and 2,742-byte precise ArcCos paths.
